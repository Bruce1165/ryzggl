...
namespace ZYRYJG
{
    ...
    public class UIHelp
    {
        ...
        /// <param name="FirstName">附加首项名称</param>
        /// <param name="FirtValue">附加首项值</param>
        /// <param name="filterStr">过滤条件（前缀不加and）</param>
        public static void FillDropDownList(RadComboBox rcb, string typeID, string FirstName, string FirtValue,string filterStr)
        {
            //DataTable dt = TypesDAL.GetList(0, int.MaxValue - 1, "and TypeID='" + typeID + "'", "SortID");
            DataTable dt = TypesDAL.GetList(0, int.MaxValue - 1, string.Format("and TypeID='{0}' and {1}", typeID, filterStr), "SortID");
            if (rcb == null) return;
            if (dt == null) return;
            if (dt.Rows.Count == 0) return;
        ...
        public static void FillYearDropDownList(RadComboBox rcb, bool defaultItemDisplay, int year = 2009)
        {
            if (rcb == null)
                return;
        ...
        /// <param name="ctl">控件</param>
        /// <param name="ReadOnly">是否只读</param>
        public static void SetReadOnly(RadioButtonList ctl, bool ReadOnly)
        {
            ctl.Enabled = !ReadOnly;
            if (ReadOnly == true)
            {
        ...
        /// <param name="ctl">控件</param>
        /// <param name="ReadOnly">是否只读</param>
        public static void SetReadOnly(CheckBoxList ctl, bool ReadOnly)
        {
            ctl.Enabled = !ReadOnly;
            if (ReadOnly == true)
            {
        ...
        public static void SetData(GridEditableItem container, object _Object, bool readOnly = false)
        {
        ...
        /// <param name="_Object">数据实体类</param>
        /// <param name="readOnly">是否将控件设置为只读</param>
        public static void SetData(HtmlControl container, object _Object, bool readOnly = false)
        {
            if (_Object == null) return;
        ...
        public static T CopyProperties<T>(T obj) where T : new()
        {
            T newObj = new T();
        ...
        public static void layerAlert(Page page, string message, int ico, int CloseTime)
        {
            ScriptManager.RegisterStartupScript(page, page.GetType(), "alert",
                string.Format(@"var scrollHeight =parent.document.documentElement.scrollHeight|| parent.document.body.scrollHeight;var scrollTop =parent.document.documentElement.scrollTop || parent.document.body.scrollTop;layer.alert('{0}',{{offset:scrollTop +(scrollHeight -scrollTop)/3,icon:{1},time:{2},area: ['600px', 'auto']}});", message.Replace(string.Format("{0}", (char)10), "").Replace(string.Format("{0}", (char)13), ""), ico, CloseTime), true);
        ...
        /// <summary>
        /// 弹出提示（特别提示，当使用 isfresh=true;做关闭刷新时，CloseTime需要传递0，不自动关闭，否则刷新不起作用）
        /// </summary>
        /// <param name="page">页面引用</param>
        /// <param name="message">提示信息</param>
        /// <param name="ico">图标：0:叹号,1:对勾,2：叉叉，3：问号，4：锁头，5：哭脸，6：笑脸</param>
        ...
        {
            ScriptManager.RegisterStartupScript(page, page.GetType(), "alert",
                string.Format(@"var scrollHeight =parent.document.documentElement.scrollHeight|| parent.document.body.scrollHeight;var scrollTop =parent.document.documentElement.scrollTop || parent.document.body.scrollTop;layer.alert('{0}',{{offset:scrollTop +(scrollHeight -scrollTop)/3,icon:{1},time:{2},area: ['800px', 'auto'],yes:function(index){{{3}layer.close(index);}}}});", message.Replace(string.Format("{0}", (char)10), "").Replace(string.Format("{0}", (char)13), ""), ico, CloseTime, exeScript), true);

        }


        /// <summary>
        /// 弹出提示（message可包含html标签，若其中带引号请使用转义字符：\'）
        /// </summary>
        /// <param name="page">页面引用</param>
        /// <param name="message">提示信息</param>
        ...
        {            
            ScriptManager.RegisterStartupScript(page, page.GetType(), "alert",
               string.Format("layer.open( {{title:'系统提示',content:'{0}',offset:'150px',area: [ '800px','auto']}});", message.Replace(string.Format("{0}", (char)10), "").Replace(string.Format("{0}", (char)13), "")), true);
        }
        ...
        //public static void layerTips(string message)
        //{
        //    ScriptManager.RegisterStartupScript((Page)HttpContext.Current.Handler, HttpContext.Current.GetType(), "layerTips",
        //       string.Format("layer.msg( '{0}');", message), true);
        ...
        public static void ShowMsgAndRedirect(System.Web.UI.Page page, string message, List<ResultUrl> listUrl)
        {
            ...
            {
                fi = new System.IO.FileInfo(page.Server.MapPath(GetUrlNoParam(r.Url)));
                size = fi.Length / 1024;
                if (size < 1024)
                    resultMessage.Append("<tr><td align=\"left\"><a href=\"").Append(AddUrlReadParam(r.Url.Replace("~", BasePage.RootUrl))).Append("\" runat=\"server\"><nobr>").Append(r.FunctionName).Append("（").Append(size.ToString()).Append(" KB）</nobr></a></td></tr>");
                else
                    resultMessage.Append("<tr><td align=\"left\"><a href=\"").Append(AddUrlReadParam(r.Url.Replace("~", BasePage.RootUrl))).Append("\" runat=\"server\"><nobr>").Append(r.FunctionName).Append("（").Append((size / 1024).ToString()).Append(" MB）</nobr></a></td></tr>");
            }
        ...
        public static string ToFullWidth(string halfWidthStr)
        {
            ...
            {
                if (c[i] >= 0x0020 && c[i] <= 0x7EFF)
                {
                    // 对应全角字符的Unicode码点比半角字符的码点增加了0xFEE0
                    c[i] = (char)(c[i] + 0xFEE0);
                    if (c[i] > 0xFF5E)
                        c[i] = (char)(c[i] - 0x0060);
                }
        ...
        public static string ShowFaceImageJZS(string PSN_RegisterNO, string WorkerCertificateCode)
        {
            ...
            if (serverType == "ww" && File.Exists(System.Web.HttpContext.Current.Server.MapPath(imgPath)) == true)
            {
                return imgPath;
            }
            else
        ...
        public static string ShowFaceImage(string WorkerCertificateCode)
        {
            ...
            //System.Random rm = new Random();
            //string img = string.Format("~/EXamManage/ExamSignimage.aspx?r={0}&o={1}", rm.Next(), Utility.Cryptography.Encrypt(imgPath)); //绑定照片;
            //return img;


            System.Random rm = new Random();
            if (MyWebConfig.serverType == "zw")//专网
            {
                return string.Format("{2}/PersonnelFile/GetImg.aspx?r={0}&w={1}", rm.Next().ToString(), HttpContext.Current.Server.UrlEncode(Utility.Cryptography.Encrypt(string.Format("{0},{1}", WorkerCertificateCode,DateTime.Now.ToString("yyyyMMddHHmmff")))), MyWebConfig.wwRoot);
            }
            else
            {
                return string.Format("~/PersonnelFile/GetImg.aspx?r={0}&w={1}", rm.Next().ToString(), HttpContext.Current.Server.UrlEncode(Utility.Cryptography.Encrypt(string.Format("{0},{1}", WorkerCertificateCode, DateTime.Now.ToString("yyyyMMddHHmmff")))));
            }            
        }
        ...
        /// <param name="WorkerCertificateCode">证件号码</param>
        /// <returns>一寸照片</returns>
        public static string ShowFaceImgByExamPlanIDAndWorkerCertificateCode(string ExamPlanID, string WorkerCertificateCode)
        {
            System.Random rm = new Random();
            if (MyWebConfig.serverType == "zw")//专网
            {
                return string.Format("{2}/PersonnelFile/GetImg.aspx?r={0}&e={1}", rm.Next().ToString(), HttpContext.Current.Server.UrlEncode(Utility.Cryptography.Encrypt(string.Format("{0},{1}", ExamPlanID, WorkerCertificateCode))), MyWebConfig.wwRoot); 
            }
            else
            {
                return string.Format("~/PersonnelFile/GetImg.aspx?r={0}&e={1}", rm.Next().ToString(), HttpContext.Current.Server.UrlEncode(Utility.Cryptography.Encrypt(string.Format("{0},{1}", ExamPlanID, WorkerCertificateCode)))); 
            }            
        }
        ...
        /// 根据证书一寸照片和证书编号获取一寸照片
        /// </summary>
        /// <param name="FacePhoto">证书一寸照片地址</param>
        /// <param name="WorkerCertificateCode">证件号码</param>
        /// <returns></returns>
        public static string ShowFaceImeByFacePhotoAndWorkerCertificateCode(string FacePhoto, string WorkerCertificateCode)
        {
            System.Random rm = new Random();
            if (MyWebConfig.serverType == "zw")//专网
            {
                return string.Format("{2}/PersonnelFile/GetImg.aspx?r={0}&f={1}", rm.Next().ToString(), HttpContext.Current.Server.UrlEncode(Utility.Cryptography.Encrypt(string.Format("{0},{1}", FacePhoto, WorkerCertificateCode))), MyWebConfig.wwRoot);
            }
            else
            {
                return string.Format("~/PersonnelFile/GetImg.aspx?r={0}&f={1}", rm.Next().ToString(), HttpContext.Current.Server.UrlEncode(Utility.Cryptography.Encrypt(string.Format("{0},{1}", FacePhoto, WorkerCertificateCode))));
            }  
        }
        ...
        /// 获取附件本机URL，（外网或专网）
        /// </summary>
        /// <param name="url">附件相对地址（以“~/”开头的地址）</param>
        /// <returns>附件地址</returns>
        public static string ShowFile(string url)
        {
            ...
            if (File.Exists(System.Web.HttpContext.Current.Server.MapPath(url)) == true)
            {
                //read参数用于核验访问网站附件DenyFileAccessHandler。
                return serverType == "ww" ? string.Format("{0}/{1}?read={2}", ww, url.Substring(2, url.Length - 2), radom) : string.Format("{0}/{1}?read={2}", zw, url.Substring(2, url.Length - 2), radom);
            }
            else
            {
                return serverType == "zw" ? string.Format("{0}/{1}?read={2}", ww, url.Substring(2, url.Length - 2), radom) : string.Format("{0}/{1}?read={2}", zw, url.Substring(2, url.Length - 2), radom);
            }
        }
        ...
        /// <summary>
        /// 基础数据库数据访问一：http://210.75.213.142/SQSWeb_Silverlight/Exchage/InterFaceService.asmx
        /// </summary>
        /// <param name="page"></param>
        /// <param name="iServiceID">服务ID（对应表）</param>
        /// <param name="pageIndex">起始页码,从0开始</param>
        /// <param name="pageSize">每页条数</param>
        /// <param name="condition">过滤条件</param>
        /// <param name="sortText">排序</param>
        /// <param name="resultCount">查询结果总数</param>
        /// <returns>查询结果Dataset</returns>
        public static DataSet QueryFromBaseDB(System.Web.UI.Page page, string iServiceID, int pageIndex, int pageSize, string condition, string sortText, out int resultCount)
        {
            //拆迁员变更限制
            ...
            System.Data.DataSet ds = ifs.PageQueryData(userName, userPassword, iServiceID, "dt", pageIndex + 1, pageSize, condition, sortText, out resultCount);
            return ds;
        }
        ...
        /// <returns></returns>
        public static int QueryCaiQianCountLimitOfUnit(System.Web.UI.Page page, string unitCode)
        {
            int resultCount = 0;
            DataSet ds = QueryFromBaseDB(page, "e047d8f6-6427-4be9-b04b-749450ba4dea", 0, 1, string.Format("ZZJGDM like '%{0}%'", unitCode), "", out resultCount);

            if (ds != null && ds.Tables[0].Rows.Count > 0)
            {
                return Convert.ToInt32(ds.Tables[0].Rows[0]["ZDRSYQ"]);
            }
            return 0;
        }

        /// <summary>
        /// 基础数据库数据访问二：http://210.75.213.238/SQSWeb_Silverlight/Exchage/InterFaceService.asmx
        /// </summary>
        ...
        /// <param name="resultCount">查询结果总数</param>
        /// <returns>查询结果Dataset</returns>
        public static DataSet QueryFromBaseDB238(string userName, string password, string iServiceID, string tableName, int pageIndex, int pageSize, string condition, string sortText, out int resultCount)
        {
            //拆迁员变更限制
            Synergy.Common.DESCrypto descObj = new Synergy.Common.DESCrypto();
            BaseDataService.InterFaceService ifs = new BaseDataService.InterFaceService();
            //System.Data.DataSet ds = ifs.QueryData(userName, userPassword, "94adf9c7-d8bd-48e4-93d7-1a827c831595", "RY_ZTSD", pageIndex, pageSize, string.Format(" 1=1 {0}", q.ToWhereString()), "", out resultCount);
            System.Data.DataSet ds = ifs.PageQueryData(userName, password, iServiceID, tableName, pageIndex + 1, pageSize, condition, sortText, out resultCount);
            return ds;
        }
        ...
        /// <returns>如果未被锁定返回空字符串，否则返回被锁定原因</returns>
        public static string QueryCertificateLockFromBaseDB(string postName, string certificateCode)
        {
            int resultCount = 0;
            Synergy.Common.DESCrypto descObj = new Synergy.Common.DESCrypto();
            ...
            string userName = descObj.EncryptString("RTDL_RYKWXT");
            string userPassword = descObj.EncryptString("RTDL_2013");
            DataSet ds = QueryFromBaseDB238(userName, userPassword, "94adf9c7-d8bd-48e4-93d7-1a827c831595", "RY_ZTSD", 0, 1, string.Format("GWGZ='{0}' and ZSBH='{1}'", postName, certificateCode), "SDSJ desc", out resultCount);

            if (ds != null && ds.Tables[0].Rows.Count > 0 && ds.Tables[0].Rows[0]["SDZT"].ToString() == "加锁")
            {
                return string.Format("编号“{0}”的{1}证书被锁定，无法办理此业务。<br />锁定时间：{2}<br />锁定业务事项：{3}<br />说明：{4}<br />备注：{5}<br />"
                    , certificateCode, postName
                    , Convert.ToDateTime(ds.Tables[0].Rows[0]["SDSJ"]).ToString("yyyy-MM-dd")
                    , ds.Tables[0].Rows[0]["SDYWSX"].ToString()
                     , ds.Tables[0].Rows[0]["SDYWSM"].ToString()
                      , ds.Tables[0].Rows[0]["BZ"].ToString()
                    );
            }
            return "";
        }
        ...
        /// 黑名单报名限制配置
        /// </summary>
        public static string GetBlackLimit(System.Web.UI.Page page, string keyName)
        {
            System.Collections.Generic.Dictionary<string, string> BlackLimit = null;

            if (page.Cache["BlackLimit"] == null)
            {
                DataTable dt = TypesDAL.GetListByTypeID("107");//黑名单报名限制配置 
                BlackLimit = new Dictionary<string, string>();
                foreach (DataRow item in dt.Rows)
                {
                    BlackLimit.Add(item["TYPENAME"].ToString(), item["TYPEVALUE"].ToString());
                }
                page.Cache["BlackLimit"] = BlackLimit;
            }
            else
            {
                BlackLimit = page.Cache["BlackLimit"] as Dictionary<string, string>;
            }

            if (BlackLimit.ContainsKey(keyName) == true)
                return BlackLimit[keyName];
            else
                return "";
        }
        ...
        /// <summary>
        /// 判断是在北京否存在有效的一级或二级建造师
        /// </summary>
        /// <param name="zjhm">证件号码</param>
        /// <returns></returns>
        public static bool IfExistJZSInBeijing(string zjhm)
        {
            int countZCJZS = CommonDAL.SelectRowCount(
                string.Format(@"select count(*) from (
	                                select 1 as c from [jcsjk_jzs] where [PSN_CertificateNO]='{0}' and PSN_Level='一级'
	                                union
	                                select 1 as c from [dbo].[COC_TOW_Person_BaseInfo] where [PSN_CertificateNO]='{0}' and [PSN_RegisteType] < '07'
                                ) t", zjhm));
            if (countZCJZS > 0)
                return true;
            else
        ...
        /// 上传社保比对数据
        /// </summary>
        /// <param name="CertificateChangeOID"></param>
        private void Upload_SBData(long CertificateChangeOID)
        {
            CertificateChangeOB ob = CertificateChangeDAL.GetObject(CertificateChangeOID);
            Synergy.Common.DESCrypto descObj = new Synergy.Common.DESCrypto();
            string userName = descObj.EncryptString("RTDL_RYKWXT");
            string userPassword = descObj.EncryptString("RTDL_2013");
            ...
            {
                BaseDataService.InterFaceService ifs = new BaseDataService.InterFaceService();
                ds = ifs.GetIServiceStruct(userName, userPassword, "274cd127-a543-4985-829f-b98f21a020e8");//获取服务的数据结构
                ...
                for (int i = 1; i < 4; i++)//近3个月
                {
                    for (int j = 1; j < 6; j++)//5个险种
                    {
                        ...
                        dr["LB"] = "03";//类别
                        dr["XM"] = ob.NewWorkerName;//姓名
                        dr["SFZHM"] = ob.NewWorkerCertificateCode;//身份证号码
                        dr["ZZJGDM"] = ob.NewUnitCode.Substring(0, 8);//组织结构代码，8位
                        dr["DWMC"] = ob.NewUnitName;//单位名称
                        dr["XZ"] = j.ToString("00");//保险种类（01~05）
                        dr["YF"] = ob.ApplyDate.Value.AddMonths(-i).ToString("yyyyMM");//缴费月份，YYYYMM
                        dr["SBYF"] = ob.ApplyDate.Value.ToString("yyyyMM");//申报月份，YYYYMM    
                        dr["JFJS"] = 0;//缴费基数
                        dr["DWJFJE"] = 0;//单位缴费金额
                        dr["GRJFJE"] = 0;//个人交费金额
                    }
                ...
                flag = ifs.AddData(userName, userPassword, "274cd127-a543-4985-829f-b98f21a020e8", ds);
            }
            catch (Exception ex)
            {
                Utility.FileLog.WriteLog("上传社保比对数据失败！", ex);
                return;
            }
        }
        ...
        /// <returns></returns>
        public static bool IsWebShell(Stream fileContent)
        {
            var fileContentString = new StreamReader(fileContent, Encoding.Default).ReadToEnd();
            Utility.FileLog.WriteLog(fileContentString);
            var webShellKeywords = new[]
                {
                    "eval", "base64_decode", "system", "exec", "passthru", "shell_exec",
                    "proc_open", "curl", "curl_exec", "file_put_contents", "fopen", "fwrite"
                };

            foreach (var keyword in webShellKeywords)
            {
                if (fileContentString.Contains(keyword))
                {
                    return true;
                }
            }
        ...
        ///// <param name="fileContent"></param>
        ///// <returns></returns>
        //public static bool IsWebShell(string filePath)
        //{
        //    //var fileContentString = new StreamReader(File.OpenRead(filePath), Encoding.UTF8).ReadToEnd();
        //    //var fileContentString = File.ReadAllText(filePath, Encoding.GetEncoding("gb2312"));
        //    var fileContentString = File.ReadAllText(filePath, Encoding.Default);
        //    Utility.FileLog.WriteLog(fileContentString);
        //    var webShellKeywords = new[]
        //        {
        //            "eval", "base64_decode", "system", "exec", "passthru", "shell_exec",
        ...
        /// <summary>
        /// 判断二建续期申请单证书现单位是否与在施锁定单位不一致
        /// </summary>
        /// <param name="ApplyType">申请类型</param>
        /// <param name="PSN_RegisterNo">证书注册编号</param>
        /// <param name="ENT_Name">企业名称</param>
        /// <returns>现单位与在施锁定单位不一致返回true，否则返回false</returns>
        public static bool IfZSSD_ContinueChangeUnit(string ApplyType, string PSN_RegisterNo, string ENT_Name)
        {
            if (ApplyType != "延期注册")
            {
                return true;//不是延期注册的在施锁定，都显示
            }
            else//延期注册
            {
                DataTable dt = CommonDAL.GetDataTable(string.Format(@"SELECT top 1 [ID],[XM],[ZJHM],[ZCH],[HTBH],[XMMC],[ZBQY],[SDZT],[SDSJ],[BZ],[VALID],[CJSJ],[XGSJ],[CJR],[XGR],[CJDEPTID],[XGDEPTID],[CheckCode]
                                                          FROM [dbo].[jcsjk_RY_JZS_ZSSD]
                                                          where [ZCH]='{0}' and [SDZT]=1 order by [SDSJ] desc", PSN_RegisterNo));

                //续期单位与在施锁定单位不一致才显示在施锁定记录，一致不显示续期的在施锁定记录
                if (dt != null && dt.Rows.Count > 0 && dt.Rows[0]["ZBQY"].ToString().Replace("(", "（").Replace(")", "）") != ENT_Name.Replace("(", "（").Replace(")", "）"))
                {
                    return true;
                }
                else
        ...
        //将半角转换为全角（忽略0-9，a-z，A-Z）
        public static string ToHalfCode(string strInput)
        {
        ...
        差值65248,空格比较特殊,全角为12288,半角为32 */
            ...
            for (int i = 0; i < c.Length; i++)
            {
                ...
                else if ((c[i] > 32 && c[i] < 48) || (c[i] > 57 && c[i] < 65) || (c[i] > 90 && c[i] < 97) || (c[i] > 122 && c[i] < 127))
                {
                    c[i] = (char)(c[i] + 65248);
                }
        ...
        /// <summary>
        /// 格式化继续校验形式，1：面授；2：网络教育；3：面授+网络教育
        /// </summary>
        /// <param name="code">继续校验形式编号</param>
        /// <returns>继续校验形式名称</returns>
        public static string GetJxjyType(object code)
        {
            if(code == null) return "";
        ...
        public static DataRow GetGFSX(string PostTypeName, string PostName, string ApplyType, string REGION)
        {
            ...
            switch (PostTypeName)
            {
                ...
                case "建筑施工特种作业":
                    ...
                    {
                        case "考核":
                            TASKNAME = string.Format("建筑施工特种作业人员职业资格认定{0}（考核）", PostName.IndexOf("（") > 0 ? string.Format("[{0}]", PostName.Replace("建筑起重机械安装拆卸工（塔式起重机）", "建筑起重机械安全拆卸工（塔式起重机）")) : string.Format("（{0}）", PostName));
                            break;
                        case "续期":
                            TASKNAME = string.Format("建筑施工特种作业人员职业资格认定{0}（延期复核）", PostName.IndexOf("（") > 0 ? string.Format("[{0}]", PostName.Replace("建筑起重机械安装拆卸工（塔式起重机）", "建筑起重机械安全拆卸工（塔式起重机）")) : string.Format("（{0}）", PostName));
                            break;
                        case "注销":
                            TASKNAME = string.Format("建筑施工特种作业人员职业资格认定{0}（注销）", PostName.IndexOf("（") > 0 ? string.Format("[{0}]", PostName.Replace("建筑起重机械安装拆卸工（塔式起重机）", "建筑起重机械安全拆卸工（塔式起重机）")) : string.Format("（{0}）", PostName));
                            break;
                    }
                    break;
                case "建设职业技能岗位":
            ...
            if (System.Web.HttpContext.Current.Cache["GFSX"] != null)
            {
                dt = (DataTable)System.Web.HttpContext.Current.Cache["GFSX"];
            }
            else
            {
                ...
                System.Web.HttpContext.Current.Cache.Insert("GFSX", dt, null, System.Web.Caching.Cache.NoAbsoluteExpiration, TimeSpan.FromHours(24));
            }
