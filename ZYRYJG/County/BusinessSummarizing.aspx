<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="BusinessSummarizing.aspx.cs" Inherits="ZYRYJG.County.BusinessSummarizing" %>
                            ...
                            <MasterTableView NoMasterRecordsText=" 没有可显示的记录" DataKeyNames="ReportCode,ENT_City,ApplyType,ApplyTypeSub" CommandItemDisplay="None" CommandItemStyle-HorizontalAlign="Left">
                                <Columns>
                                    ...
                                    <telerik:GridTemplateColumn UniqueName="Edit" HeaderText="操作">
                                        <ItemTemplate>
                                            <a class="link" style="color: #0000ff" target="_blank" href="<%# ZYRYJG.UIHelp.ShowFile(string.Format("~/Upload/ReportXls/Excel{0}.xls",Eval("ReportCode"))) %>" title="点击导出或右键另存">导出</a>
                                            <%#Eval("ReportStatus").ToString()=="未上报"?"<span class=\"link\" onclick=\"javascript:tips_pop('" +Eval("ReportCode").ToString() +"')\">上传汇总扫描件</span>":""%>
                                            <input id="Button1" runat="server" type="button" value="修改" class="link" onclick='<%# string.Format("javascript:SetIfrmSrc(\"ChoiceBusinessSummarizing.aspx?o1={0}&o2={1}&o3={2}\");return false;",Eval("ENT_City"),Eval("ReportCode"),Eval("ApplyType"))%>' visible='<%#Eval("ReportStatus").ToString()=="未上报" %>' />
                                            ...
                                            <%#IfUploadReportImg(Eval("ReportCode").ToString())==true?string.Format("<a class=\"link\" target=\"_blank\" style=\"color: #0000ff; margin-left:20px\"   href=\"./ReportImgView.aspx?o={0}\">预览</a>",Eval("ReportCode")):"" %>
                                            ...
                                            <input id="Button6" runat="server" type="button" value="详细" class="link" onclick='<%# string.Format("javascript:SetIfrmSrc(\"ChoiceBusinessSummarizing.aspx?o1={0}&o2={1}&o3={2}&o4={3}\");return false;",Eval("ENT_City"),Eval("ReportCode"),Eval("ApplyType"),"详细")%>' visible='<%#Eval("ReportStatus").ToString()=="已上报" %>' />
                                        </ItemTemplate>
