<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Qualification.aspx.cs" Inherits="ZYRYJG.County.Qualification" %>
...
<head runat="server">
    ...
    <script type="text/javascript">
        ...
        function Update(zgzsbh)
        {
            ...
            $.post("../Ajax/Qualification.ashx",{action:'select',dzgzsbh:zgzsbh},function(result){
                if(result!="False")
                {
            ...
            $("#BtnSave").click(function(){
                layer.confirm('是否修改?', {icon: 3, title:'提示'}, function(index){
                    $.post("../Ajax/Qualification.ashx",{action:'update',dzgzsbh:zgzsbh,XM:$("#TxtXm").val(),ZJHM:$("#TxtZjhm").val(),GZDW:$("#TxtGzdw").val(),ZGZSBH:$("#TxtZgzsbh").val(),QDNF:$("#TxtQdnf").val(),ZYLB: $("#TxtZylb").val(),QFSJ: $("#TxtQfsj").val(),ZGZH: $("#TxtZgzh").val(),BYXX: $("#TxtByxx").val(),BYSJ: $("#TxtBysj").val(),SXZY: $("#TxtSxzy").val(),ZGXL: $("#TxtZgxl").val()},function(result){
        ...
        function Delete(zgzsbh) {
            layer.confirm('是否删除?', {icon: 3, title:'提示'}, function(index){
                $.post("../Ajax/Qualification.ashx",{action:'delete',dzgzsbh:zgzsbh},function(result){
                    if(result=="True")
                    {
        ...
        function UpdateZgNo(zgzsbh,zjhm)
        {
            ...
            $.post("../Ajax/Qualification.ashx",{action:'selectCert',zjhm:zjhm},function(result){
                if(result!="False")
                {
            ...
            $("#BtnSave").click(function(){
                layer.confirm('是否修改证书?', {icon: 3, title:'提示'}, function(index){
                    $.post("../Ajax/Qualification.ashx",{action:'updateCert',zjhm:zjhm,PSN_Name:$("#TxtXm").val(),PSN_CertificateNO:$("#TxtZjhm").val(),PSN_Sex: $("#TxtSex").val(),ZGZSBH: $("#TxtZgzh2").val()},function(result){
                        //$.post("../Ajax/Qualification.ashx",{action:'update',XM:$("#TxtXm").val(),ZJHM:$("#TxtZjhm").val(),GZDW: $("#TxtGzdw").val(),ZYLB: $("#TxtZylb").val(),ZGZSBH: $("#TxtZgzh").val()},function(result){
                        if(result=="True")
                        {
                            layer.closeAll(); 
            ...
            <div  style="width: 99%; margin: 5px auto; padding-bottom: 30px;">
                <div id="div_top" class="dqts">
                    <div style="float: left;">
                        <img alt="" src="../Images/ts.gif" width="17" height="11" />当前位置 &gt;&gt;建造师注册管理 &gt;&gt;<strong>资格库管理</strong>
                    </div>
                </div>
                <div class="content">
                    <div style="width: 95%; height: 100%; margin: 10px auto; text-align: center;">
                                ...
                                <Columns>
                                    ...
                                    <telerik:GridTemplateColumn UniqueName="Edit" HeaderText="操作">
                                        <ItemTemplate>
                                            ...
                                            <input id="Button3" type="button" value="更新证书" class="link" onclick="return UpdateZgNo('<%# Eval("ZGZSBH")%>    ','<%#Eval("ZJHM") %>    ')" />
                                        </ItemTemplate>
                        ...
                        <asp:ObjectDataSource ID="ObjectDataSource1" runat="server" DataObjectTypeName="Model.QualificationDAL"
                            SelectMethod="GetList" TypeName="DataAccess.QualificationDAL"
