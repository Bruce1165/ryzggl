...
{
    //汇总
    public partial class ChoiceBusinessSummarizing : BasePage
    {
        protected override string CheckVisiteRgihtUrl
        {
            get
            {
                return "County/BusinessSummarizing.aspx";
            }
        }
        ...
        protected void ButtonSearch_Click(object sender, EventArgs e)
        {
            ...
            q.Add(string.Format("PSN_Level = '{0}'",RadioButtonListPSN_Level.SelectedValue));//注册等级
           
            //判断他是否详细
            if (ViewState["ReportCode"] != null && ViewState["XiangXi"]!=null)//edit
            {
                //公示批次号
                q.Add(string.Format("(ReportCode ='{0}')", ViewState["ReportCode"].ToString()));
                //详细就把这两个控件隐藏掉
            ...
            {
                //公示批次号
                q.Add(string.Format("(ReportCode ='{0}' or (ApplyStatus = '{1}' and ReportCode is null)) AND ApplyType = '{2}'", ViewState["ReportCode"].ToString(), EnumManager.ApplyStatus.区县审查, ViewState["ApplyType"].ToString()));
                tableSearch.Visible = false;
            }
            //判断他是否是新增
            else//new
        ...
        protected void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            string ReportCode= ViewState["ReportCode"]!=null?ViewState["ReportCode"].ToString():ApplyDAL.GetNextReportCode(Region
            ...
            {              
                if (ViewState["ReportCode"]!=null)//修改
                {
                    ...
                    //取消已分配批次号
                    ApplyDAL.UpdatePatchReport(tran, ReportCode);

                    //重新分配批次号
                    ApplyDAL.SavePatchReport(tran, ReportCode, selectApplyIDs);
                    tran.Commit();
                }
                else//新增
                {
                    ...
                    if (RadComboBoxIten.SelectedValue =="PSN_Level" && RadTextBoxValue.Text.Trim() =="二级"||RadTextBoxValue.Text.Trim() =="二级临时")
                    {
                        ViewState["PSN_Level"] = RadTextBoxValue.Text.Trim();
                    }
                ...
                ButtonSearch_Click(sender, e);
                UIHelp.WriteOperateLog(UserName, UserID, "区县汇总成功", string.Format("受理时间：{0}", DateTime.Now));
            ...
            {
                tran.Rollback();
                UIHelp.WriteErrorLog(Page, "保存汇总的信息失败", ex);
                return;
            }
        ...
        protected void CreateExcel(string ReportCode)
        {
            ...
            if (ViewState["ApplyType"].ToString() == "初始注册")
            {
                ...
                column = @"row_number() over(order by [dbo].[Apply].[ApplyID])\PSN_Name\PSN_CertificateNO\ENT_Name\(select  '【' +ZGZSBH + '，' + ZYLB+ '，' +CONVERT(varchar(100), QFSJ, 23)+'】' from [Qualification] where ZJHM = apply.PSN_CertificateNO and PATINDEX ( '%'+ZYLB+'%' , apply.PSN_RegisteProfession ) >0 FOR XML PATH(''))\ExamineResult\ExamineRemark";
                string Caption = @"
<tr style=""height:30pt"">
    <td style=""font-weight:bold;text-align:center;"" class=""noborder"" colspan=""7"">北京市二级建造师{0}初审意见汇总表</td>
</tr>
<tr>
...
<tr>
    ...
    <td class=""noborder""></td>  
                ...
                CommonDAL.OutputXls(HttpContext.Current.Server.MapPath(filePath)
                    ...
                    , head.ToString()
                    , column.ToString()
                    //, string.Format(Caption, ViewState["ApplyType"], ReportCode, DateTime.Now.ToString("yyyy-MM-dd"), colspan, colspan - 1, colspan + 3,"区县")
            ...
            #region 重新注册
            if (ViewState["ApplyType"].ToString() == "重新注册")
            {
                //数据表的列明
                string column = "";

                column = @"row_number() over(order by [dbo].[Apply].[ApplyID])\PSN_Name\PSN_CertificateNO\ENT_Name\PSN_RegisterNo\(select  '【' +ZGZSBH + '，' + ZYLB+ '，' +CONVERT(varchar(100), QFSJ, 23)+'】' from [Qualification] where ZJHM = apply.PSN_CertificateNO and PATINDEX ( '%'+ZYLB+'%' , apply.PSN_RegisteProfession ) >0 FOR XML PATH(''))\ExamineResult\ExamineRemark";
                string Caption = @"
<tr style=""height:30pt"">
    <td style=""font-weight:bold;text-align:center;"" class=""noborder"" colspan=""8"">北京市二级建造师{0}初审意见汇总表</td>
</tr>
...
<tr style=""height:30pt"">
    <td style=""text-align:left;"" colspan=""3"" class=""noborder"">制表人（签字）：</td>
    <td style=""text-align:left;"" class=""noborder"" colspan=""2"">审核人（签字）：</td>
    <td style=""text-align:left;"" class=""noborder"" colspan=""1"">审核日期：</td>
...
<tr>
    ...
    <td class=""noborder""></td>
    <td style=""width:320pt"" class=""noborder""></td>
    <td style=""width:100pt"" class=""noborder""></td>
    <td style=""width:100pt"" class=""noborder""></td>  
</tr>";
                //EXCEL表头明
                string head = "";
                head = @"序号\姓名\证件号码\企业名称\注册号\执业资格证书编号/管理号_专业_签发时间\审核结果\审核意见"; 
                //过滤条件
                string filterSql = string.Format(" and ReportCode ='{0}'", ReportCode);
                if (!System.IO.Directory.Exists(HttpContext.Current.Server.MapPath("~/Upload/ReportXls/")))
                {
            ...
            #region 增项注册
            if (ViewState["ApplyType"].ToString() == "增项注册")
            {
                //数据表的列明
                string column = "";

                column = @"row_number() over(order by [dbo].[Apply].[ApplyID])\PSN_Name\PSN_CertificateNO\ENT_Name\PSN_RegisterNo\(select  ISNULL(a.AddItem1,'')+ISNULL(+','+a.AddItem2,'')  from  ApplyAddItem a where ISNULL(a.AddItem1,'')+ISNULL(a.AddItem2,'') !='' and a.ApplyID=[Apply].ApplyID)\(select isnull(a.ExamCode1,'')+ISNULL(+','+a.ExamCode2,'') from  ApplyAddItem a where ISNULL(a.AddItem1,'')+ISNULL(a.AddItem2,'') !='' and a.ApplyID=[Apply].ApplyID)\ExamineResult\ExamineRemark";
                string Caption = @"
<tr style=""height:30pt"">
    <td style=""font-weight:bold;text-align:center;"" class=""noborder"" colspan=""8"">北京市二级建造师{0}初审意见汇总表</td>
</tr>
...
<tr style=""height:30pt"">
    ...
    <td style=""text-align:left;"" class=""noborder"" colspan=""2"">部门公章</td>
                ...
                //EXCEL表头明
                string head = "";
                head = @"序号\姓名\证件号码\企业名称\注册号\增项专业\增项专业执业资格编号/管理号\审核结果\审核意见";
                //过滤条件
                string filterSql = string.Format(" and ReportCode ='{0}'", ReportCode);
                if (!System.IO.Directory.Exists(HttpContext.Current.Server.MapPath("~/Upload/ReportXls/")))
                {
            ...
            #region 延期注册
            if (ViewState["ApplyType"].ToString() == "延期注册")
            {
                //数据表的列明
                string column = "";

                column = @"row_number() over(order by [dbo].[Apply].[ApplyID])\PSN_Name\PSN_CertificateNO\ENT_Name\PSN_RegisterNo\(select CONVERT(varchar(100),PSN_CertificateValidity, 23) from COC_TOW_Person_BaseInfo a where a.PSN_CertificateNO=Apply.PSN_CertificateNO and a.psn_registetype<'07')\PSN_RegisteProfession\ExamineResult\ExamineRemark";
                string Caption = @"
<tr style=""height:30pt"">
    <td style=""font-weight:bold;text-align:center;"" class=""noborder"" colspan=""8"">北京市{4}建造师{0}初审意见汇总表</td>
</tr>
<tr>
    <td style=""text-align:left;"" colspan=""3"" class=""noborder"">批次号：{1}</td>
    <td style=""text-align:center;"" colspan=""2"" class=""noborder"">区县：{3}</td>
...
<tr style=""height:30pt"">
    ...
    <td style=""text-align:left;"" class=""noborder"" colspan=""2"">部门公章</td>
                ...
                CommonDAL.OutputXls(HttpContext.Current.Server.MapPath(filePath)
                    ...
                    , column.ToString()
                    //, string.Format(Caption, ViewState["ApplyType"], ReportCode, DateTime.Now.ToString("yyyy-MM-dd"), colspan, colspan - 1, colspan + 3,"区县")
                    , string.Format(Caption, ViewState["ApplyType"], ReportCode, DateTime.Now.ToString("yyyy-MM-dd"), ViewState["ENT_City"], RadioButtonListPSN_Level.SelectedValue)
                    , Foot
                );
            ...
            #region 遗失补办
            if (ViewState["ApplyType"].ToString() == "遗失补办")
            {
                //数据表的列明
                string column = "";
...
<tr>
    <td style=""text-align:left;"" colspan=""3"" class=""noborder"">批次号：{1}</td>
    <td style=""text-align:cenetr;"" colspan=""2"" class=""noborder"">区县：{3}</td>
    <td style=""text-align:right;"" class=""noborder"" colspan=""3"">创建日期：{2}</td>
</tr>";
...
<tr style=""height:30pt"">
    ...
    <td style=""text-align:left;"" class=""noborder"" colspan=""2"">部门公章</td>
            ...
            if (ViewState["ApplyType"].ToString() == "执业企业变更")
            {
                ...
                column = @"row_number() over(order by [dbo].[Apply].[ApplyID])\PSN_Name\PSN_CertificateNO\ENT_Name\PSN_RegisterNo\(select CONVERT(varchar(100),PSN_CertificateValidity, 23) from COC_TOW_Person_BaseInfo a where a.PSN_CertificateNO=Apply.PSN_CertificateNO and a.psn_registetype<'07')\PSN_RegisteProfession\(select OldENT_Name from ApplyChange b where b.ApplyID=Apply.ApplyID)\ExamineResult\ExamineRemark";
                string Caption = @"
<tr style=""height:30pt"">
    <td style=""font-weight:bold;text-align:center;"" class=""noborder"" colspan=""8"">北京市{4}建造师{0}初审意见汇总表</td>
</tr>
...
<tr style=""height:30pt"">
    ...
    <td style=""text-align:left;"" class=""noborder"" colspan=""2"">部门公章</td>
            ...
            if (ViewState["ApplyType"].ToString() == "个人信息变更")
            {
                ...
                column = @"row_number() over(order by [dbo].[Apply].[ApplyID])\PSN_Name\PSN_CertificateNO\ENT_Name\PSN_RegisterNo\(select CONVERT(varchar(100),PSN_CertificateValidity, 23) from COC_TOW_Person_BaseInfo a where a.PSN_CertificateNO=Apply.PSN_CertificateNO and a.psn_registetype<'07' and a.psn_level='二级')\PSN_RegisteProfession\ExamineResult\ExamineRemark";
                string Caption = @"
<tr style=""height:30pt"">
    <td style=""font-weight:bold;text-align:center;"" class=""noborder"" colspan=""8"">北京市{4}建造师{0}初审意见汇总表</td>
</tr>
...
<tr style=""height:30pt"">
    ...
    <td style=""text-align:left;"" class=""noborder"" colspan=""2"">部门公章</td>
            ...
            if (ViewState["ApplyType"].ToString() == "企业信息变更")
            {
                ...
                column = @"row_number() over(order by [dbo].[Apply].[ApplyID])\PSN_Name\PSN_CertificateNO\ENT_Name\(select ENT_NameTo from ApplyChange a where a.ApplyID=apply.ApplyID)\PSN_RegisterNo\(select CONVERT(varchar(100),PSN_CertificateValidity, 23) from COC_TOW_Person_BaseInfo a where a.PSN_CertificateNO=Apply.PSN_CertificateNO and a.psn_registetype<'07')\PSN_RegisteProfession\ExamineResult\ExamineRemark";
                string Caption = @"
<tr style=""height:30pt"">
    <td style=""font-weight:bold;text-align:center;"" class=""noborder"" colspan=""8"">北京市{4}建造师{0}初审意见汇总表</td>
</tr>
