...
namespace ZYRYJG.EXamManage
{
    ...
    /// 证书打印（作废）
    /// </summary>
    public partial class NewCertificatePrintManage : BasePage
    {
        ...
        protected void ButtonSearch_Click(object sender, EventArgs e)
        {
            ...
            //{
            //    
            //    q.Add(string.Format("Status = '{0}'", EnumManager.CertificateUpdateType.first));//证书更新状态(首次)
            ...
            if (TrContinueParm.Visible == true)
            {
                ...
                {
                    q.Add(string.Format("CertificateID in(select CertificateID from dbo.CertificateContinue where ConfirmCode like '%{0}%')", RadTextBoxConfirmCode.Text.Trim()));
                }
                //续期决定时间
                if (RadDatePickerConfirmDateStart.SelectedDate.HasValue || RadDatePickerConfirmDateEnd.SelectedDate.HasValue)
                {
                    q.Add(string.Format("CertificateID in(select CertificateID from dbo.CertificateContinue where ( ConfirmDate BETWEEN  '{0}' AND '{1}'))"
            ...
            if (string.IsNullOrEmpty(Request["o"]) != true && Request.QueryString["t"].ToString() == "Continue")//续期
            {
                ...
                sortStr1.FieldName = "UnitCode,CertificateID";
        ...
        protected void ButtonPrint_Click(object sender, EventArgs e)
        {
            ...
            Response.Redirect(string.Format("NewCertificatePrint.aspx?CertificateID=&rtn_o={0}&rnt_t={1}",
        ...
        protected void ButtonCaseUpdate_Click(object sender, EventArgs e)
        {
            ...
            UIHelp.WriteOperateLog(PersonName, UserID, "证书打印归档", string.Format("归档证书数量：{0}本。"
                    , count.ToString()));
            UIHelp.layerAlert(Page, "证书归档成功",6,3000);
            ClearGridSelectedKeys(RadGridCertificate);
        ...
        protected List<Dictionary<string, string>> GetPrintData()
        {
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                printData.Add("Img_FacePhoto_" + cellNo.ToString(), GetFacePhotoPath(dt.Rows[i]["ExamPlanID"].ToString(), dt.Rows[i]["WorkerCertificateCode"].ToString()));
            }
        ...
        //{
        //    if (CertificateCode == "") return "../Images/null.jpg";
        ...
        /// <returns></returns>
        public string GetFacePhotoPath(string ExamPlanID, string WorkerCertificateCode)
        {
        ...
        protected void RadGridCertificate_ItemDataBound(object sender, GridItemEventArgs e)
        {
            if (e.Item.ItemType == GridItemType.Item || e.Item.ItemType == GridItemType.AlternatingItem)
            {
                ...
                e.Item.Cells[RadGridCertificate.MasterTableView.Columns.FindByUniqueName("DetailView").OrderIndex].Attributes.Add("onclick"
                    ,string.Format("location='NewCertificatePrint.aspx?CertificateID={0}&rtn_o={1}&rnt_t={2}';",
        ...
        //根据证件号码显示照片
        protected string ShowFaceimage(string WorkerCertificateCode, string ExamPlanID)
        {
            System.Random rm = new Random();
            string img = string.Format("~/EXamManage/ExamSignimage.aspx?r={0}&o={1}", rm.Next().ToString(), Utility.Cryptography.Encrypt(GetFacePhotoPath(ExamPlanID,WorkerCertificateCode))); //绑定照片;
            return img;
        }
    }
}
