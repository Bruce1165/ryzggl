...
{
    public partial class ExamPlan : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (ValidResourceIDLimit(RoleIDs, "ExamSignSplit") == true)//拆分考试计划权限
                {
                    RadGridExamPlan.MasterTableView.Columns.FindByUniqueName("split").Visible = true;
                }
        ...
        protected void RadGridExamPlan_ItemDataBound(object sender, GridItemEventArgs e)
        {
            ...
            {
                //绑定培训点列表
                DataTable dtTrainUnit = UserDAL.GetList(0, int.MaxValue - 1, " and OrganID=245", "RelUserName");
                ...
                if (e.Item.OwnerTableView.IsItemInserted)//new
                {
                    ...
                    //HtmlTableRow tr = e.Item.FindControl("TrExamPlanSubject") as HtmlTableRow;
                    //tr.Visible = false;//隐藏考试科目

                    //绑定科目                    
                    DataTable dt = new DataTable();// PostInfoDAL.GetListByPostID(_PostSelect.PostTypeID);
                    RadGrid RadGridSubject = e.Item.FindControl("RadGridExamPlanSubject") as RadGrid;
                ...
                else//update
                {
                    ...
                    //if (Convert.ToDateTime(DateTime.Now.ToString("yyyy-MM-dd")) > ExamPlanDAL.GetObject(id).SignUpEndDate.Value)
                    //{
                    ...
                    RadioButton RadioButtonPublish = e.Item.FindControl("RadioButtonPublish") as RadioButton;
                    RadioButton RadioButtonUnPublish = e.Item.FindControl("RadioButtonUnPublish") as RadioButton;
                    RadioButton RadioButtonTrainUnitLimit = e.Item.FindControl("RadioButtonTrainUnitLimit") as RadioButton;//培训点受限
                    RadComboBox RadComboBoxPLANSKILLLEVEL = (RadComboBox)e.Item.FindControl("RadComboBoxPLANSKILLLEVEL");
                    RadioButtonList RadioButtonListExamWay= (RadioButtonList)e.Item.FindControl("RadioButtonListExamWay");//考试方式

                    RadioButtonListExamWay.Items.FindByValue(ob.ExamWay).Selected = true;

                    //职业技能岗位考试计划有“技术等级”分类(除村镇建筑工匠、普工之外)
                    if (_PostSelect.PostTypeID == "4"
                        && _PostSelect.PostID != "158"//村镇建筑工匠
                        && _PostSelect.PostID != "199"//普工
                        )
                    {
                    ...
                    switch (ob.IfPublish)
                    {
                        case EnumManager.ExamPlanPublishType.Publish://"完全公开"
                            RadioButtonPublish.Checked = true;
                            RadioButtonUnPublish.Checked = false;
                            RadioButtonTrainUnitLimit.Checked = false;
                            break;
                        case EnumManager.ExamPlanPublishType.UnPublish://部分公开
                            RadioButtonPublish.Checked = false;
                            RadioButtonUnPublish.Checked = true;
                            RadioButtonTrainUnitLimit.Checked = false;
                            break;
                        case EnumManager.ExamPlanPublishType.PublishAndTrainUnitLimit://完全公开培训点受限
                            RadioButtonPublish.Checked = false;
                            RadioButtonUnPublish.Checked = false;
                            RadioButtonTrainUnitLimit.Checked = true;
                            break;
                    }
                    SetCtlVissible(editedItem);
        ...
        private List<ExamPlanForUserOB> GetExamPlanForUserOBList(long ExamPlanID, GridEditableItem e)
        {
            ...
            foreach (RadListBoxItem li in rlbTrain.Items)
            {
                if (li.Checked == true)
                {
                    ...
                    ob.UserType = "培训点";
                    ob.TrainUnitID = Convert.ToInt64(li.Value);
                    list.Add(ob);
                }
            }

            //企业
            foreach (RadListBoxItem li in rlbUnitCode.Items)
            {
                ExamPlanForUserOB ob = new ExamPlanForUserOB();
                ob.ExamPlanID = ExamPlanID;
                ob.UserType = "企业";
                ob.UnitCode = li.Value;
                list.Add(ob);
            }
        ...
        {
            GridItem editedItem;//= (e.Item is GridCommandItem) ? e.Item as GridEditableItem : RadGridExamPlan.MasterTableView.GetInsertItem();// (e.Item as GridDataItem).Parent;
            if (e.Item is GridDataItem)
                //editedItem = e.Item.Parent.Controls[e.Item.ItemIndex + 1 + 2] as GridEditFormItem;
            ...
            switch (e.CommandName)
            {
                ...
                case "AddIDCard"://添加可见证件号码
                    ...
                    //if (rtbIDCard.Text.Trim() == "") return;
                    //if (rlbIDCard.FindItemByValue(rtbIDCard.Text.Trim()) != null)
                    //{
                    //    UIHelp.layerAlert(Page, "已经添加过了，不能重复添加！");
                    //    return;
                    //}
                    //if (Utility.Check.isChinaIDCard(rtbIDCard.Text.Trim()) == false)
                    //{
                    //    UIHelp.layerAlert(Page, "身份证号不正确，无法添加！");
                    //    return;
                    //}
                    //rlbIDCard.Items.Add(new RadListBoxItem(rtbIDCard.Text.Trim(), rtbIDCard.Text.Trim()));
        ...
        protected void RadGridExamPlan_InsertCommand(object source, GridCommandEventArgs e)
        {
            ...
            RadDatePicker PayNoticeEndDate = editedItem.Cells[0].FindControl("PayNoticeEndDate") as RadDatePicker;
            ...
            RadDatePicker RadDatePickerLatestPayDate = e.Item.FindControl("RadDatePickerLatestPayDate") as RadDatePicker;
            RadioButton RadioButtonPublish = e.Item.FindControl("RadioButtonPublish") as RadioButton;
            RadioButton RadioButtonUnPublish = e.Item.FindControl("RadioButtonUnPublish") as RadioButton;
            RadioButton RadioButtonTrainUnitLimit = editedItem.FindControl("RadioButtonTrainUnitLimit") as RadioButton;//培训点受限
            ...
            RadNumericTextBox RadNumericTextBoxPersonLimit =(RadNumericTextBox)editedItem.FindControl("RadNumericTextBoxPersonLimit");
            RadioButtonList RadioButtonListExamWay = editedItem.FindControl("RadioButtonListExamWay") as RadioButtonList;//考试方式
            ...
            //职业技能岗位（除村镇建筑工匠、普工、塔机拆装工）必须选择技术等级
            if (_PostSelect.PostTypeID == "4"
                && listPostID.Contains("158")==false//村镇建筑工匠
                && listPostID.Contains("199")==false//普工
                 && listPostID.Contains("1120") == false//物业管理员
                && RadComboBoxPLANSKILLLEVEL.SelectedValue == "")
            {
            ...
            if (RadDatePickerLatestCheckDate.SelectedDate > RadDatePickerLatestPayDate.SelectedDate)
            {
                UIHelp.layerAlert(Page, "审核截止日期不应大于缴费截止日期！");
                e.Canceled = true;
                return;
            }
            if (RadDatePickerLatestPayDate.SelectedDate > PickerExamCardSendStartDate.SelectedDate)
            {
                UIHelp.layerAlert(Page, "缴费截止日期不应大于准考证发放开始时间！");
                e.Canceled = true;
                return;
            }
            ...
            ////工种集合
            //DataTable tdPost = PostInfoDAL.GetList(0, int.MaxValue - 1, " and UpPostID=" + _PostSelect.PostTypeID, "PostID");
            ...
            if (RadioButtonPublish.Checked == true)
                ob.IfPublish = "完全公开";
            else if (RadioButtonUnPublish.Checked == true)
                ob.IfPublish = "部分公开";
            else if (RadioButtonTrainUnitLimit.Checked == true)
                ob.IfPublish = "完全公开培训点受限";

            ob.PersonLimit = Convert.ToInt32(RadNumericTextBoxPersonLimit.Value);
            ob.ExamWay = RadioButtonListExamWay.SelectedValue;
            ...
            try
            {
                for (int i = 0; i < listPostID.Count; i++)//循环工种
                {
                    ...
                    if (ob.IfPublish == "部分公开" || ob.IfPublish == "完全公开培训点受限")
                    {
        ...
        //变换考试计划可见类型时触发
        protected void RadioButtonPublish_OnCheckedChanged(object sender, EventArgs e)
        {
            GridEditableItem editedItem;
            if (RadGridExamPlan.EditItems.Count > 0)//update
            {
                editedItem = (sender as RadioButton).NamingContainer as GridEditableItem;
            }
            else//new
            {
                editedItem = RadGridExamPlan.MasterTableView.GetInsertItem();
            }
            SetCtlVissible(editedItem);
        }

        //设定控件可见性
        protected void SetCtlVissible(GridEditableItem editedItem)
        {
            RadioButton RadioButtonPublish = editedItem.FindControl("RadioButtonPublish") as RadioButton;//完全公开
            RadioButton RadioButtonUnPublish = editedItem.FindControl("RadioButtonUnPublish") as RadioButton;//部分公开
            RadioButton RadioButtonTrainUnitLimit = editedItem.FindControl("RadioButtonTrainUnitLimit") as RadioButton;//培训点受限
            ...
            RadTextBox rtbUnitCode = editedItem.FindControl("RadTextBoxUnitCode") as RadTextBox;
            RadTextBox rtbIDCard = editedItem.FindControl("RadTextBoxIDCard") as RadTextBox;
            Button ButtonAddUnitCode = editedItem.FindControl("ButtonAddUnitCode") as Button;
            Button ButtonDeleteUnitCode = editedItem.FindControl("ButtonDeleteUnitCode") as Button;
            Button ButtonAddIDCard = editedItem.FindControl("ButtonAddIDCard") as Button;
            Button ButtonDeleteIDCard = editedItem.FindControl("ButtonDeleteIDCard") as Button;
            if (RadioButtonPublish.Checked == true)//完全公开
            {
                rlbTrain.Enabled = false;
                rlbUnitCode.Enabled = false;
                rtbUnitCode.Enabled = false;
                rlbIDCard.Enabled = false;
                rtbIDCard.Enabled = false;
                ButtonAddUnitCode.Enabled = false;
                ButtonDeleteUnitCode.Enabled = false;
                ButtonAddIDCard.Enabled = false;
                ButtonDeleteIDCard.Enabled = false;
            }
            else if (RadioButtonUnPublish.Checked == true)//部分公开
            {
                rlbTrain.Enabled = true;
                rlbUnitCode.Enabled = true;
                rtbUnitCode.Enabled = true;
                rlbIDCard.Enabled = true;
                rtbIDCard.Enabled = true;
                ButtonAddUnitCode.Enabled = true;
                ButtonDeleteUnitCode.Enabled = true;
                ButtonAddIDCard.Enabled = true;
                ButtonDeleteIDCard.Enabled = true;
            }
            else if (RadioButtonTrainUnitLimit.Checked == true)//完全公开,但培训点受限
            {
                rlbTrain.Enabled = true;
                rlbUnitCode.Enabled = false;
                rtbUnitCode.Enabled = false;
        ...
        protected void PostSelect1_OnPostTypeSelectChange(object source, EventArgs e)
        {
            if (RadGridExamPlan.MasterTableView.GetInsertItem() != null)//new
            {
                ...
                PostSelect _PostSelect = (PostSelect)editedItem.FindControl("PostSelect1");
                RadNumericTextBox _RadNumericTextBoxExamFee = (RadNumericTextBox)editedItem.FindControl("RadNumericTextBoxExamFee");
                RadComboBox RadComboBoxPLANSKILLLEVEL = (RadComboBox)editedItem.FindControl("RadComboBoxPLANSKILLLEVEL");
                string _PostTypeID = _PostSelect.PostTypeID == "" ? "-1" : _PostSelect.PostTypeID;
                DataTable dt = PostInfoDAL.GetListByPostID(_PostTypeID);
                if (_PostTypeID == "-1")
                    _RadNumericTextBoxExamFee.Value = 0;//收费金额
                else
                {
                    PostInfoOB _PostInfoOB = PostInfoDAL.GetFirtPostByPostTypeID(Convert.ToInt32(_PostTypeID));
                    _RadNumericTextBoxExamFee.Value = Convert.ToDouble(_PostInfoOB.ExamFee.Value);
                }
                RadGrid RadGridSubject = editedItem.FindControl("RadGridExamPlanSubject") as RadGrid;
                RadGridSubject.DataSource = dt;
                RadGridSubject.DataBind();
