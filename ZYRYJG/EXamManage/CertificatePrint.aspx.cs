...
namespace ZYRYJG.EXamManage
{
    ...
    /// 证书打印（作废）
    /// </summary>
    public partial class CertificatePrint : BasePage
    {
        ...
        protected void ButtonSearch_Click(object sender, EventArgs e)
        {
            ...
            if (ViewState["ChangeType"] !=null && ViewState["ChangeType"].ToString() == "Continue" && PostSelect1.PostTypeID == "2")//续期打印特种作业，满足6年换证时间才打印
            {
                q.Add("((DATEPART(year,ValidEndDate) - DATEPART(year,ConferDate) -2)% 6 =0)");
            }
        ...
        protected void ButtonPrint_Click(object sender, EventArgs e)
        {
            ...
            DataTable dt = null;// CertificateDAL.GetList(0, int.MaxValue - 1, filterString, "CertificateCode");

            if (string.IsNullOrEmpty(Request["o"]) != true && Request.QueryString["t"].ToString() == "Continue")//续期
            {
                dt = CertificateDAL.GetListContinuePrint(0, int.MaxValue - 1, filterString, "CONTINUE_CHECKDATE,CONTINUE_GETDATE,UNITCODE,POSTID,CERTIFICATECODE");
            }
            else
            {
            ...
            Response.Redirect(string.Format("CertificateInfo.aspx?CertificateID=&rtn_o={0}&rnt_t={1}",
        ...
        protected void RadGridCertificate_ItemDataBound(object sender, GridItemEventArgs e)
        {
            if (e.Item.ItemType == GridItemType.Item || e.Item.ItemType == GridItemType.AlternatingItem)
            {
                ...
                e.Item.Cells[RadGridCertificate.MasterTableView.Columns.FindByUniqueName("DetailView").OrderIndex].Attributes.Add("onclick"
                    , string.Format("location='CertificateInfo.aspx?CertificateID={0}&rtn_o={1}&rnt_t={2}';",
