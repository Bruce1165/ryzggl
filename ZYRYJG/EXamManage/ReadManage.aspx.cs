...
{
    public partial class ReadManage :BasePage
    {
        ...
        protected void RefreshGrid(int pageIndex)
        {
            ...
            if (RadTextBoxExamPlaceName.Text.Trim() != "") queryExamResult.Add(string.Format("ExamPlaceName like '%{0}%'", RadTextBoxExamPlaceName.Text.Trim()));//考点名称
            if (RadTextBoxExamRoomCode.Text.Trim() != "") queryExamResult.Add(string.Format("ExamRoomCode ='{0}'", RadTextBoxExamRoomCode.Text.Trim()));//考场号
            if (RadTextBoxWorkerName.Text.Trim() != "") queryExamResult.Add(string.Format("WorkerName like '%{0}%'", RadTextBoxWorkerName.Text.Trim()));//姓名
            if (RadTextBoxExamCardID.Text.Trim() != "") queryExamResult.Add(string.Format("ExamCardID = '{0}'", RadTextBoxExamCardID.Text.Trim()));//准考证
            if (RadTextBoxCertificateCode.Text.Trim() != "") queryExamResult.Add(string.Format("CertificateCode like '%{0}%'", RadTextBoxCertificateCode.Text.Trim()));//个人证件号码
            if (RadTextBoxTrainUnit.Text.Trim() != "") queryExamResult.Add(string.Format("TrainUnitName like '%{0}%'", RadTextBoxTrainUnit.Text.Trim()));//培训点名称
            if (RadTextBoxUnit.Text.Trim() != "") queryExamResult.Add(string.Format("UnitName like '%{0}%'", RadTextBoxUnit.Text.Trim()));//单位名称
            if (RadTextBoxExamCardID.Text.Trim() != "")//输入准考证查询条件可以不输入考试计划条件
            {
                if (_ExamPlanID !="0") queryExamResult.Add("ExamPlanID=" + _ExamPlanID);// 考试计划
            }
            else
            {
                queryExamResult.Add("ExamPlanID=" + _ExamPlanID);// 考试计划
            }
            ...
            DataTable dtMain = ExamResultDAL.GetListView(pageIndex * RadGridExamSubjectResult.PageSize, RadGridExamSubjectResult.PageSize, queryExamResult.ToWhereString(), sortString);
            ...
            if (ExamPlanSelect1.PostTypeID.HasValue && ExamPlanSelect1.PostTypeID.Value == 2)
            {
                AddBoundColumn("电话", "S_PHONE");
            }
            ...
            if (dtMain.Rows.Count > 0)
            {
                ...
                DataTable dtScore = ExamSubjectResultDAL.GetList(0, int.MaxValue - 1
                    , string.Format(" and ExamPlanID ={0} and ExamCardID in({1})", _ExamPlanID, sb.ToString())
                    , "ExamCardID");
        ...
        protected void ButtonImportScore_Click(object sender, EventArgs e)
        {
            ...
            int KeMuId = PostInfoDAL.GetObject(ExamPlanSelect1.PostID.Value, RadComboBoxPostTypeID.SelectedItem.Text).PostID.Value;//考试科目id

             //设定合格线后不能再次导入成绩
            int count = ExamPlanSubjectDAL.SelectCount(string.Format(" and ExamPlanID={0} and PostID={1} and PassLine is not null", ExamPlanSelect1.ExamPlanID.ToString(), KeMuId));
            if (count > 0)
            {
        ...
        {           
           ...
           // 删除空行
            ...
            if (RadTextBoxExamRoomCode.Text.Trim() != "") queryExamResult.Add(string.Format("ExamRoomCode ='{0}'", RadTextBoxExamRoomCode.Text.Trim()));//考场号
            if (RadTextBoxWorkerName.Text.Trim() != "") queryExamResult.Add(string.Format("WorkerName like '%{0}%'", RadTextBoxWorkerName.Text.Trim()));//姓名
            if (RadTextBoxExamCardID.Text.Trim() != "") queryExamResult.Add(string.Format("ExamCardID like '%{0}%'", RadTextBoxExamCardID.Text.Trim()));//准考证
            if (RadTextBoxCertificateCode.Text.Trim() != "") queryExamResult.Add(string.Format("CertificateCode like '%{0}%'", RadTextBoxCertificateCode.Text.Trim()));//个人证件号码
            ...
            int gongZhongID = ExamPlanDAL.GetObject(ExamPlanSelect1.ExamPlanID.Value).PostID.Value;//工种ID
            long explanID = ExamPlanSelect1.ExamPlanID.Value;//考试计划id
            int KeMuId = PostInfoDAL.GetObject(gongZhongID, kemuName).PostID.Value;//考试科目id

            try
            {
                //清空原科目成绩
                ExamSubjectResultDAL.DeleteByKeMuID(tran, explanID, KeMuId);
                ...
                for (int m = 0; m < dt.Rows.Count; m++)
                {
                    ...
                    examSubjectResultOB.ExamPlanID = explanID;   //考试计划id
                    ...
                    examSubjectResultOB .CreatePersonID =PersonID ;  //创建人
                ...
                ExamPlanSelect1.ExamPlanName, kemuName));   
        ...
        //导入成绩模板下载
        protected void LinkButtonDownLoadScoreTemplate_Click(object sender, EventArgs e)
        {
            if (ExamPlanSelect1.ExamPlanID.HasValue == false)
            {
                UIHelp.layerAlert(this.Page, "必须选择一个考试计划名称！");
            ...
            //输出
            Utility.ExcelDealHelp.ExportExcel(Page, filePath,"成绩_" + ExamPlanSelect1.PostName +".xls");
        }
