...
function FloatAd(selector) {
    ...
    var windowHeight = 300;//$(window).height();//浏览器高度
    ...
    obj.css({ left: "1px", top: "20px" });//把元素设置成在页面中间
    ...
    function move() {//定期执行的函数，使元素移动
        ...
        if (nextPosX <= 0 || nextPosX >= (windowWidth - 100) / 2) {//如果达到左边，或者达到右边，则改变为相反方向
            dirX = dirX * -1;//改变方向
            nextPosX = currentPos.left + dirX;//为了不过界，重新获取下一个位置
        }
        if (nextPosY <= 20 || nextPosY >= 200) {//如果达到上边，或者达到下边，则改变为相反方向。            
            dirY = dirY * -1;//改变方向
