...
namespace ZYRYJG.CertifEnter
{
    public partial class CertifEnterApplyEdit : BasePage
    {
        ...
        protected override string CheckVisiteRgihtUrl
        {
            get
            {
                return "CertifEnterApplyList.aspx|CertifEnterCheck.aspx|CertifEnterAccepted.aspx|ApplyQuerySLR.aspx";
            }
        }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (string.IsNullOrEmpty(Request["o"]) == false)//修改
                {
                    ...
                    {
                        if (_CertificateOutMDL.out_name != _CertificateEnterApplyOB.WorkerName)
                        {
                            UIHelp.layerAlert(Page, string.Format("注意：系统检测到申请单填写姓名“{0}”与外省转出证书信息“{1}”不一致。", _CertificateEnterApplyOB.WorkerName, _CertificateOutMDL.out_name));
                            return;
                        }
                        if (_CertificateOutMDL.out_expiringDate != _CertificateEnterApplyOB.ValidEndDate)
                        {
                            UIHelp.layerAlert(Page, string.Format("注意：系统检测到申请单填写证书有效期至“{0}”与外省转出证书信息“{1}”不一致。", _CertificateEnterApplyOB.ValidEndDate, _CertificateOutMDL.out_expiringDate));
                            return;
                        }
                        string pname = "";
                        switch(_CertificateOutMDL.out_categoryCode)
                        {
                            case "A":
                                pname = "企业主要负责人";
                                break;
                        ...
                        if (pname != PostSelect1.PostName)
                        {
                            UIHelp.layerAlert(Page, string.Format("注意：系统检测到申请单填写岗位工种{0}与外省转出证书编号{1}不对应。", PostSelect1.PostName, _CertificateEnterApplyOB.CertificateCode));
                            return;
                        }
...
//                    }
        ...
        {            
            ...
            {
                DateTime myBirthday = RadDatePickerBirthday.SelectedDate.Value;//生日
                ...
                if (PostSelect1.PostTypeID == "1")
                {
                    ...
                    {                       
                        ...
                        if (RadComboBoxJob.SelectedItem.Text == "法定代表人")//职务
                        {
                            ...
                            {
                                UIHelp.layerAlert(Page, "校验未通过：工商信息显示您并非进京单位企业法人，不能以法定代表人职务进京。");
                                return;
                            }

                            CertificateOB FROb = CertificateDAL.GetFRCertA(RadTextBoxCreditCode.Text.Trim());
                            if (FROb != null)
                            {
                                UIHelp.layerAlert(Page, string.Format("校验未通过：一个单位只能允许一人以法定代表人职务持有A证，进京单位已经存在法人A证【{0}】。", FROb.CertificateCode));
                                return;
                            }
                            //法人不受年龄显限制
                        }
                        ...
                        {
                            //非企业法人，受年龄限制
                            if (Utility.Check.CheckBirthdayLimit(Convert.ToInt32(PostSelect1.PostID), RadTextBoxWorkerCertificateCode.Text.Trim(), myBirthday, mySex) == true)
                            {
                                UIHelp.layerAlert(Page, "您已超龄,不予进京！");
                                return;
                            }

                            CertificateOB FROb = CertificateDAL.GetOtherNoFRCertA(RadTextBoxWorkerCertificateCode.Text.Trim(), RadTextBoxCertificateCode.Text.Trim());
                            if (FROb != null)
                            {
                ...
                {
                    if (Utility.Check.CheckBirthdayLimit(Convert.ToInt32(PostSelect1.PostTypeID), RadTextBoxWorkerCertificateCode.Text.Trim(), myBirthday, mySex) == true)
                    {
                        UIHelp.layerAlert(Page, "您已超龄，不予进京！");
                        return;
                    }
            ...
            if (CertificateDAL.CheckRegular_ItemMaster(RadTextBoxWorkerCertificateCode.Text.Trim(), UnitCode, Convert.ToInt32(PostSelect1.PostID), RadComboBoxJob.SelectedItem.Text) == false)
            {
                UIHelp.layerAlert(Page, "同时持有多本安管人员ABC证书的，其项目负责人B本、专职安管人员C本应与其一本法人A本证书工作单位一致。只允许在同一单位同时取得“项目负责人”和“专职安全生产管理人员(C1、C2、C3)”证书，并且有C3不能再报考C1和C2，有C1或C2不能报考C3，您不符合要求，不能申报！");
                return;
            }
...
";
        ...
        private void BindCheckHistory(long ApplyId)
        {
            DataTable dt =((PersonType == 2 || PersonType == 3)?
                CertificateEnterApplyDAL.GetCheckHistoryListForGRQY(ApplyId)
                :CertificateEnterApplyDAL.GetCheckHistoryList(ApplyId));
            ...
            switch (ApplyStatus)
            {
                ...
                case EnumManager.CertificateEnterStatus.Decided://已编号
                    try
                    {
                        ...
                        if (_CertificateOB != null && _CertificateOB.ZZUrlUpTime > _CertificateOB.CheckDate)
                        {
        ...
        protected void ShowZACheckResult()
        {
            ...
            if (ViewState["CertificateEnterApplyOB"] != null)
            {
                ...
                <div style='padding-left: 32px'>警告：进京申请没有通过<a href='https://zlaq.mohurd.gov.cn/fwmh/bjxcjgl/fwmh/pages/default/index.html' target='_blank'>【全国工程质量安全监管信息平台（可按身份证查询ABC持证情况）】</a>数据校验，属于违规持证。<br/>
