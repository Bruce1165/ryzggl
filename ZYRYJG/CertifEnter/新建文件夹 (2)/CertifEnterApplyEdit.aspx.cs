...
//                    }
        ...
        {            
            ...
            //9、证书有效期截止日期不低于三天，低于三天提示：“您的证书有效期低于3个工作日（办理进京业务审批时限），不能申请进京业务。

            if (RadDatePickerValidEndDate.SelectedDate.Value < Convert.ToDateTime(DateTime.Now.AddDays(3).ToString("yyyy-MM-dd")))
            {
                UIHelp.layerAlert(Page, "您的证书有效期低于3个工作日（办理进京业务审批时限），不能申请进京业务。", 5, 0);
                return;
            }
                ...
                )
             {
                 ...
                 if (ButtonExit.Text == "提交单位审核")
                 {
                     ...
                     fj = new System.Collections.Hashtable{
                        ...
                        {EnumManager.FileDataTypeName.安全生产考核证书扫描件,0}                        
                    };
...
";
        ...
        protected void ButtonUnitCheck_Click(object sender, EventArgs e)
        {
            if (RadDatePickerValidEndDate.SelectedDate.Value < Convert.ToDateTime(DateTime.Now.AddDays(3).ToString("yyyy-MM-dd")))
            {
                UIHelp.layerAlert(Page, "证书有效期低于3个工作日（办理进京业务审批时限），不能申请进京业务。", 5, 0);
                return;
            }
