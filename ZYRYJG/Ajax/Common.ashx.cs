...
namespace ZYRYJG.Ajax
{
    ...
    public class Common : IHttpHandler, IRequiresSessionState
    {
        ...
        {
            if (string.IsNullOrEmpty(context.Request["code"]) == true)
            {
                context.Response.Write("用户集成认证失败！");
            ...
            string grant_code = context.Request["code"];
            ...
            try
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        //context.Response.Write(string.Format("获取 accessToken 成功,{0}", AccessToken));
                        ...
                        //context.Response.Write(string.Format("userInfo：{0}", rtn));
                        ...
                        try
                        {
                            ...
                            if (checkdate == false)
                            {
                                ...
                                context.Response.Write("用户集成认证失败！出生日期格式不正确。");
                                return;
                            }
                        }
                        catch
                        ...
                        {
                            context.Response.Write(string.Format(@"<p>系统要求必须达到L3或以上实名制级别才允许登录系统办理业务，请到<a href=""http://www.beijing.gov.cn/\"">首都之窗</a> 进行实名制认证升级。  <br /> <br /> 
                                 L1：未认证，匿名；<br /> 
                        ...
                        try
                        {
                            ...
                            {
                                context.Session["myMissExam"] = string.Format("{0}|{1}", Convert.ToDateTime(dtMissExam.Rows[0]["EXAMSTARTDATE"]).ToString("yyyy-MM-dd"), dtMissExam.Rows[0]["PostName"]);
                            }
                        }
                        catch { }
