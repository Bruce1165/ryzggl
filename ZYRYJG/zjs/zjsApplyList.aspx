<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="zjsApplyList.aspx.cs" Inherits="ZYRYJG.zjs.zjsApplyList" %>
        ...
        <div class="div_out">
            <div class="dqts">
                <div style="float: left;">
                    当前位置 &gt;&gt;二级造价工程师注册 &gt;&gt;<strong><asp:Label ID="LabelPath" runat="server" Text=""></asp:Label></strong>
                </div>
            ...
            <div class="content">
                ...
                <table class="cx" width="98%" border="0" align="center" cellspacing="5">
                    ...
                    <tr>
                        ...
                        <td align="left">
                            ...
                            &nbsp;<input id="ButtonRenew" runat="server" visible="false" type="button" value="发起重新注册" class="bt_large" onclick='javascript: SetIfrmSrc("zjsApplyRenew.aspx"); ' />
                        </td>
                    </tr>
                </table>
                ...
                <div id="div_applyDeleteTime" runat="server" visible="false" style="width: 99%; margin: 8px auto; color: red;"><%= string.Format("请在提交网上申请后，联系聘用单位进行审核上报。{0}天内单位业未在网上进行审核上报的，系统将自动删除此次业务申请。", Model.EnumManager.ApplyDeleteTime.时间间隔) %></div>
                 <div id="divPersonInfoChangeTip" runat="server" visible="false" style="width: 99%; margin: 8px auto; color: red;">多专业变更资格证书编号的需要多本证书分别申请，变更其他个人信息的只需要申请一本证书即可，第二专业证书个人信息会同步更新。</div>
                    ...
                    <telerik:RadGrid ID="RadGridQY" runat="server"
                        ...
                        <MasterTableView NoMasterRecordsText=" 没有可显示的记录" CommandItemDisplay="None" CommandItemStyle-HorizontalAlign="Left"
                            ...
                            <Columns>
                                ...
                                <telerik:GridTemplateColumn UniqueName="ENT_Name" HeaderText="单位名称">
                                    <ItemTemplate>
                                ...
                                <telerik:GridTemplateColumn UniqueName="Edit" HeaderText="操作">
                                    <ItemTemplate>                                    
                                        <span class="link_edit" onclick='javascript:SetIfrmSrc("<%# GetApplyTypeUrl( Eval("ApplyID")==DBNull.Value?ApplyType:(Eval("ApplyType").ToString().IndexOf("变更") > -1 ?Eval("ApplyTypeSub").ToString(): Eval("ApplyType").ToString())) %>?o=<%# Utility.Cryptography.Encrypt(Eval("PSN_ServerID").ToString()) %>&a=<%# Eval("ApplyID")==DBNull.Value?"":Utility.Cryptography.Encrypt(Eval("ApplyID").ToString()) %>")'>
                                            <%# formatApplyShow(ApplyType,  Eval("ApplyID")==DBNull.Value?"": Eval("ApplyID").ToString(),  Eval("ApplyStatus").ToString(),  Eval("PSN_RegisteProfession").ToString(),Convert.ToDateTime(Eval("PSN_CertificateValidity")))%>
                                            <%# Eval("ApplyID")==DBNull.Value||Eval("ApplyStatus").ToString()=="未申报"||Eval("ApplyStatus").ToString() == "已驳回"?"提交":"撤销"%>
                                            <%# Eval("ApplyID")==DBNull.Value?"":"打印"%>
                                        </span> 
                                    </ItemTemplate>
                                    <HeaderStyle HorizontalAlign="Center" Wrap="false" />
                ...
                <asp:ObjectDataSource ID="ObjectDataSource1" runat="server" DataObjectTypeName="Model.zjs_ApplyMDL"
                    SelectMethod="GetListApplyView" TypeName="DataAccess.zjs_ApplyDAL"
