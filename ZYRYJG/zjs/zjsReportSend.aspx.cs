...
{
    public partial class zjsReportSend : BasePage
    {
        //业务员上报
...
//                    try
//                    {
//                        zjs_ApplyDAL.DelPatchReport(RadGridHZSB.MasterTableView.DataKeyValues[e.Item.ItemIndex]["ReportCode"].ToString()
//                        , Region == "全市" ? "" : Region
...
//                    //    if (RadGridHZSB.MasterTableView.DataKeyValues[e.Item.ItemIndex]["ApplyTypeSub"].ToString() == "执业企业变更")
//                    //    {
//                    //        int count2 = CommonDAL.GetRowCount("zjs_Apply", "*", string.Format("and  ReportCode='{0}' and [CheckZSSD]=1", RadGridHZSB.MasterTableView.DataKeyValues[e.Item.ItemIndex]["ReportCode"]));
//                    //        if (count2 > 0)
//                    //        {
...
//                            {
//                                //更新申请单
//                                zjs_ApplyDAL.PatchReportFinish(tran
...
//                                    where [PSN_RegisterNO] in(select [PSN_RegisterNO] from [dbo].[zjs_Apply] where ReportCode='{0}' and ExamineResult='通过')";
...
//                                        where [zjs_Apply].ReportCode='{0}' and [zjs_Apply].ExamineResult='通过'";
...
//                                    where [zjs_Apply].ReportCode='{0}' and [zjs_Apply].ExamineResult='通过'", RadGridHZSB.MasterTableView.DataKeyValues[e.Item.ItemIndex]["ReportCode"]));
...
//                                //                                    ,'BGSQ' + [zjs_Apply].ApplyCode [APPLYCODE]
//                                //                                    ,dateadd(s,1,getdate()) [GETDATE]
//                                //                                    ,'通过' [GETRESULT]
//                                //                                    ,'系统' [GETMAN]
//                                //                                    ,'BGSL' + [zjs_Apply].ApplyCode  [GETCODE]
//                                //                                    ,dateadd(s,2,getdate()) [CHECKDATE]
//                                //                                    ,'通过' [CHECKRESULT]
//                                //                                    ,'系统' [CHECKMAN]
//                                //                                    ,'BGSH' + [zjs_Apply].ApplyCode  [CHECKCODE]
//                                //                                    ,dateadd(s,3,getdate()) [CONFRIMDATE]
//                                //                                    ,'通过' [CONFRIMRESULT]
//                                //                                    ,'系统' [CONFRIMMAN]
//                                //                                    ,'BGJD' + [zjs_Apply].ApplyCode  [CONFRIMCODE]
//                                //                                    ,dateadd(s,4,getdate()) [NOTICEDATE]
//                                //                                    ,'通过' [NOTICERESULT]
//                                //                                    ,'系统' [NOTICEMAN]
//                                //                                    ,'BGGZ' + [zjs_Apply].ApplyCode  [NOTICECODE]
//                                //                                    ,'已告知' [STATUS]
//                                //                                    ,-1 [CREATEPERSONID]
//                                //                                    ,[zjs_Apply].[NoticeDate] [CREATETIME]
//                                //                                    ,-1 [MODIFYPERSONID]
//                                //                                    ,[zjs_Apply].[NoticeDate] [MODIFYTIME]
//                                //                                    ,[ApplyChange].OldENT_Name [UNITNAME]
//                                //                                    ,[ApplyChange].ENT_Name [NEWUNITNAME]
//                                //                                    ,[zjs_Apply].[OldEnt_QYZJJGDM] [UNITCODE]
//                                //                                    ,[zjs_Apply].[ENT_OrganizationsCode] [NEWUNITCODE]
...
//                                //                                    ,Convert(varchar(10),[zjs_Apply].[NoticeDate],120) + '日二建变更（申请编号：'+ [zjs_Apply].ApplyCode +'）审核通过，触发本证书同步变更到新单位。'
//                                //                                     from [dbo].[zjs_Apply] 
//                                //                                     inner join [dbo].[ApplyChange] on [zjs_Apply].ApplyID=[ApplyChange].ApplyID
//                                //                                     inner join [dbo].[CERTIFICATE] on [CERTIFICATE].POSTTYPEID=1 and [CERTIFICATE].POSTID <>147 AND [zjs_Apply].[PSN_CertificateNO]= [CERTIFICATE].WORKERCERTIFICATECODE
//                                //                                     left join (select distinct ZZJGDM from dbo.QY_BWDZZZS WHERE  qylb is not null and qylb <>'' and qylb <>'外地进京' and qymc is not null and qymc <>'') t on [zjs_Apply].ENT_OrganizationsCode = t.ZZJGDM
//                                //                                      where [zjs_Apply].ReportCode='{0}' and [zjs_Apply].ExamineResult='通过' and [zjs_Apply].[ApplyTypeSub]='执业企业变更'
//                                //                                    and [CERTIFICATE].VALIDENDDATE > getdate()  and [CERTIFICATE].[STATUS] <>'注销'  and [CERTIFICATE].[STATUS] <>'离京变更' and [CERTIFICATE].[STATUS] <>'待审批' and [CERTIFICATE].[STATUS] <>'进京待审批'
//                                //                                    and [zjs_Apply].[ENT_OrganizationsCode] <> [CERTIFICATE].[UNITCODE]
...
//                                //                                        inner join dbo.Apply on [CERTIFICATE].POSTTYPEID=1 and [CERTIFICATE].POSTID <>147 AND [CERTIFICATE].WORKERCERTIFICATECODE = [zjs_Apply].[PSN_CertificateNO]  
//                                //                                        left join (select distinct ZZJGDM from dbo.QY_BWDZZZS WHERE  qylb is not null and qylb <>'' and qylb <>'外地进京' and qymc is not null and qymc <>'') t on [zjs_Apply].ENT_OrganizationsCode = t.ZZJGDM                                      
//                                //                                        where [zjs_Apply].ReportCode='{0}' and [zjs_Apply].ExamineResult='通过' and [zjs_Apply].[ApplyTypeSub]='执业企业变更'
//                                //                                        and [CERTIFICATE].VALIDENDDATE > getdate()  and [CERTIFICATE].[STATUS] <>'注销'  and [CERTIFICATE].[STATUS] <>'离京变更' and [CERTIFICATE].[STATUS] <>'待审批' and [CERTIFICATE].[STATUS] <>'进京待审批'
//                                //                                        and [zjs_Apply].[ENT_OrganizationsCode] <> [CERTIFICATE].[UNITCODE]
...
//                                //                                            SET [CERTIFICATE].[UNITNAME]=[zjs_Apply].ENT_Name,
//                                //		                                        [CERTIFICATE].[UNITCODE]=[zjs_Apply].ENT_OrganizationsCode,
//                                //		                                        [CERTIFICATE].[MODIFYTIME]=getdate(),
//                                //                                                [CERTIFICATE].CheckDate=[zjs_Apply].[NoticeDate],
...
//                            {
//                                tran.Rollback();
//                                UIHelp.WriteErrorLog(Page, "造价工程师上报失败", ex);
//                                return;
//                            }
//                            BindRadGridHZSB();
...
//                            //{
//                            //                                //更新申请单
//                            //                                zjs_ApplyDAL.PatchReportFinish(tran1
//                            //                                 , RadGridHZSB.MasterTableView.DataKeyValues[e.Item.ItemIndex]["ReportCode"].ToString()
//                            //                                , Region
//                            //                                , RadGridHZSB.MasterTableView.DataKeyValues[e.Item.ItemIndex]["ApplyType"].ToString()
//                            //                                , UserName
...
//                            //                                        on [COC_TOW_Person_BaseInfo].[PSN_RegisterNO] = [zjs_Apply].[PSN_RegisterNO]
//                            //                                        inner join [dbo].[ApplyChange] 
//                            //                                        on [zjs_Apply].ApplyID=[ApplyChange].ApplyID
//                            //                                        where [zjs_Apply].ReportCode='{0}' and [zjs_Apply].ExamineResult='通过' and [COC_TOW_Person_BaseInfo].[ENT_OrganizationsCode] = [zjs_Apply].[ENT_OrganizationsCode]";
//                            //                                //增加过滤条件 [COC_TOW_Person_BaseInfo].[ENT_OrganizationsCode] = [zjs_Apply].[ENT_OrganizationsCode] 申办途中变更了单位，不再更新企业名称。
...
//                            //                                        FROM [dbo].[Unit] where [ENT_OrganizationsCode] in(select distinct ENT_OrganizationsCode from [dbo].[zjs_Apply] where ReportCode='{0}' and ExamineResult='通过')";
//                            //                                CommonDAL.ExecSQL(tran1, string.Format(sql, RadGridHZSB.MasterTableView.DataKeyValues[e.Item.ItemIndex]["ReportCode"]));
...
//                            //	                                    (
//                            //	                                    select distinct [zjs_Apply].[ENT_OrganizationsCode], [ApplyChange].ENT_NameTo,[ApplyChange].ToENT_City,[ApplyChange].[ToEND_Addess]
//                            //	                                    from dbo.Apply 
//                            //		                                    inner join [dbo].[ApplyChange] 
//                            //                                            on [zjs_Apply].ApplyID=[ApplyChange].ApplyID
//                            //	                                        where [zjs_Apply].ReportCode='{0}'  and ExamineResult='通过'
...
//                            try
//                            {
//                                zjs_ApplyDAL.PatchReport(RadGridHZSB.MasterTableView.DataKeyValues[e.Item.ItemIndex]["ReportCode"].ToString()
//                                , Region == "全市" ? "" : Region
//                                , RadGridHZSB.MasterTableView.DataKeyValues[e.Item.ItemIndex]["ApplyType"].ToString()
//                                , UserName
//                                );
...
//                    try
//                    {
//                        zjs_ApplyDAL.CancelPatchReport(RadGridHZSB.MasterTableView.DataKeyValues[e.Item.ItemIndex]["ReportCode"].ToString()
//                        , Region == "全市" ? "" : Region
//                        , RadGridHZSB.MasterTableView.DataKeyValues[e.Item.ItemIndex]["ApplyType"].ToString());
//                        BindRadGridHZSB();
