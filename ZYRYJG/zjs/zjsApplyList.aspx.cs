...
{
    public partial class zjsApplyList : BasePage
    {
        /// <summary>
        /// 注册类型
        /// </summary>
        ...
        public string GetApplyTypeUrl(string type)
        {
            ...
            switch (type)
            {
                ...
                case "初始注册":
                    return "zjsApplyFirst.aspx";//初始注册
                case "个人信息变更":
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (ApplyType == EnumManager.ZJSApplyType.执业企业变更)
                {
                    div30DayTip.Visible=true;
                    div_applyDeleteTime.Visible = true;
                }
                 if (ApplyType == EnumManager.ZJSApplyType.个人信息变更)
                {
                    divPersonInfoChangeTip.Visible = true;
                 }
                LabelPath.Text =  ApplyType;
                LabelTitle.Text = ApplyType ;
        ...
        protected void ButtonSearch_Click(object sender, EventArgs e)
        {
            ...
            if (RoleIDs == "0")//个人
            {
                if (ViewState["ApplyType"].ToString() == "z")//注销注册,显示未注销的个人证书
                {
                    q.Add("PSN_RegisteType < '07' ");
            ...
            {
                case "全部":
                    if (ApplyType.IndexOf("变更")>-1)
                    {
                        q.Add(string.Format("(ApplyStatus is null or ApplyTypeSub ='{0}')", ApplyType));
                    }
                    else
                    {
        ...
        public string formatApplyShow(string _ApplyType, string _ApplyID, string _ApplyStatus, string _Profession, DateTime ValidEndDate)
        {
            if (_ApplyType == "延续注册"
                 ...
                 && CheckContinueTimeSpan(_Profession, ValidEndDate) == false)
            {
                return "";
            }
            else if (string.IsNullOrEmpty(_ApplyID) == true || _ApplyStatus == "未申报")
            {
                return "申报";
            }
