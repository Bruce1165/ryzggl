...
namespace ZYRYJG
{
    public partial class pingjiaresult : BasePage
    {
        ...
        //            TASKNAME_SUB = string.Format("建筑施工特种作业人员职业资格认定（{0}）", PostName);
        //            switch (ApplyType)
        //            {
        //                case "考核":
...
where   applyid='{0}' and (a.[ApplyType]='初始注册' or a.[ApplyType]='重新注册' or a.[ApplyType]='变更注册') ", _ApplyMDL.ApplyID));
                        ...
                        foreach (DataRow r in zy.Rows)
                        {
                            ...
                            drSX = UIHelp.GetGFSX("二级注册建造师"
                                ...
                                , _ApplyMDL.ApplyType
                                , (_ApplyMDL.ApplyType == "变更注册" && _ApplyMDL.ENT_City != "丰台区" ? _ApplyMDL.ENT_City.Replace("亦庄", "经济技术开发区") : "北京市"));
                    ...
                    case "applyZJS"://二级造价工程师注册
                        ...
                        //drSX = UIHelp.GetGFSX("二级注册造价工程师", "注册造价工程师注册", _zjs_ApplyMDL.ApplyType, "北京市");//正确的

                        //**************临时替换**************************************************
                        drSX = UIHelp.GetGFSX("二级注册造价工程师", "注册造价工程师注册", _zjs_ApplyMDL.ApplyType.Replace("注销","延续注册"), "北京市");//错误的，临时将错就错
                        //******************************************************************************
        ...
        protected void SendMessage(projectServiceMDL _projectService, string DataType, string DataID, DataRow drSX)
        {
            ...
            if (_AppraiseResultMDL.success == true)
            {
                ...
                divInfo.InnerText = string.Format("评价申报业务【{0}】,编号：{1}", _projectService.taskName, _projectService.projectNo);
                CommonDAL.ExecSQL(string.Format("UPDATE [dbo].[temp_ping]   SET [DoTime] = getdate() WHERE [DataType] = '{0}' and [DataID] = '{1}' ", DataType, DataID));
            }
            else
            {
