<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ApplyFirst.aspx.cs" Inherits="ZYRYJG.Unit.ApplyFirst" %>
        ...
        <div class="div_out">
            <div class="dqts">
                <div style="float: left;">
                    当前位置 &gt;&gt;申报事项 &gt;&gt;<strong>初始注册</strong>
                </div>
                 <uc3:myhelp runat="server" ID="myhelp1" PageID="常见问题及处理方法最终版.htm" />
            </div>
            <div class="content">
                <p class="jbxxbt">
                    初始注册
                ...
                <table class="cx" width="98%" border="0" align="center" cellspacing="5">
                    ...
                    <tr>
                        ...
                        <td align="left">
                            <asp:Button ID="ButtonQuery" runat="server" Text="查询在办业务" CssClass="bt_large" OnClick="ButtonQuery_Click" />
                            &nbsp;<input id="ButtonRenew" runat="server" visible="false" type="button" value="发起初始注册" class="bt_large" onclick='javascript: SetIfrmSrc("ApplyFirstAdd.aspx"); ' />
                          <%--  <a href="/Template/企业使用手册V1.0.0.docx"  style="color:#F00;font-weight:bold; font-size:large">常见问题？</a> --%>
                        </td>
                    </tr>
                </table>
                ...
                <div style="width: 99%; margin: 8px auto;">
                    <telerik:RadGrid ID="RadGridQY" runat="server"
                        ...
                        <MasterTableView NoMasterRecordsText=" 没有可显示的记录" CommandItemDisplay="None" CommandItemStyle-HorizontalAlign="Left"
                            ...
                            <Columns>
                                ...
                                <telerik:GridTemplateColumn UniqueName="PSN_BirthDate" HeaderText="年龄">
                                    <ItemTemplate>
                                        <%# Eval("Birthday") == DBNull.Value?"":Convert.ToString(DateTime.Now.Year - Convert.ToDateTime(Eval("Birthday")).Year) %>
                                    </ItemTemplate>
                                    <HeaderStyle HorizontalAlign="Center" />
                                    <ItemStyle HorizontalAlign="Center" />
                                ...
                                <telerik:GridBoundColumn UniqueName="ApplyRegisteProfession" DataField="ApplyRegisteProfession" HeaderText="注册专业">
                                    <HeaderStyle HorizontalAlign="center" />
                                    <ItemStyle HorizontalAlign="center" />
                                </telerik:GridBoundColumn>
                                ...
                                <telerik:GridTemplateColumn UniqueName="progress" HeaderText="进度">
                                    <ItemTemplate>
                                        <%# BindApplyStatus(Eval("ApplyStatus").ToString(),(Eval("PSN_RegisterNo")==DBNull.Value?false:true),Eval("ConfirmResult").ToString())%></span>
                                    </ItemTemplate>
                                ...
                                <telerik:GridTemplateColumn UniqueName="Edit">
                                    <ItemTemplate>
                                        <%--2020-02-12 南静修改参数传参加密,源代码为不加密的参数形式--%>
                                        <span class="link_edit" onclick=' javascript:SetIfrmSrc("ApplyFirstAdd.aspx?a=<%# Utility.Cryptography.Encrypt(Eval("ApplyID").ToString()) %>"); '>详细</span>
                                     <%# Eval("ApplyStatus").ToString() == "未申报"||Eval("ApplyStatus").ToString() == "已驳回"?string.Format("<span class=\"link_edit\" onclick=' javascript:SetIfrmSrc(\"ApplyFirstAdd.aspx?a={0}\"); '>提交</span>",Utility.Cryptography.Encrypt(Eval("ApplyID").ToString())):Eval("ApplyStatus").ToString() == "已申报"?string.Format("<span class=\"link_edit\" onclick=' javascript:SetIfrmSrc(\"ApplyFirstAdd.aspx?a={0}\"); '>撤销</span>",Utility.Cryptography.Encrypt(Eval("ApplyID").ToString())):"" %>  
                                        <span class="link_edit" onclick=' javascript:SetIfrmSrc("ApplyFirstAdd.aspx?a=<%# Utility.Cryptography.Encrypt(Eval("ApplyID").ToString()) %>"); '>打印</span>
                                    </ItemTemplate>
                                    <HeaderStyle HorizontalAlign="Center" Wrap="false" />
                    ...
                    <asp:ObjectDataSource ID="ObjectDataSource1" runat="server" DataObjectTypeName="Model.ApplyFirstMDL"
                        SelectMethod="GetListView" TypeName="DataAccess.ApplyFirstDAL"
