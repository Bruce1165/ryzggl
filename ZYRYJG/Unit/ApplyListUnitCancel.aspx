<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ApplyListUnitCancel.aspx.cs" Inherits="ZYRYJG.Unit.ApplyListUnitCancel" %>
        ...
        <div class="div_out">
            <div class="dqts">
                <div style="float: left;">
                    当前位置 &gt;&gt;二级建造师注册 &gt;&gt;<strong>申请强制注销</strong>
                </div>
                        ...
                        <MasterTableView NoMasterRecordsText=" 没有可显示的记录" CommandItemDisplay="None" CommandItemStyle-HorizontalAlign="Left"
                            ...
                            <Columns>
                                ...
                                <telerik:GridTemplateColumn UniqueName="Edit" HeaderText="操作">
                                    <ItemTemplate>                                       
                                        <span class="link_edit" onclick='javascript:SetIfrmSrc("ApplyCancel.aspx?o=<%# Utility.Cryptography.Encrypt(Eval("PSN_ServerID").ToString()) %>&a=<%# Eval("ApplyID")==DBNull.Value?"":Utility.Cryptography.Encrypt(Eval("ApplyID").ToString()) %>")'>
                                        ...
                                        <span class="link_edit" onclick='javascript:SetIfrmSrc("ApplyCancel.aspx?o=<%# Utility.Cryptography.Encrypt(Eval("PSN_ServerID").ToString()) %>&a=<%# Eval("ApplyID")==DBNull.Value?"":Utility.Cryptography.Encrypt(Eval("ApplyID").ToString()) %>")'><%# Eval("ApplyID")==DBNull.Value?"":Eval("ApplyStatus").ToString()=="未申报"||Eval("ApplyStatus").ToString() == "已驳回"?"提交":"撤销"%></span>
                                        <span class="link_edit" onclick='javascript:SetIfrmSrc("ApplyCancel.aspx?o=<%# Utility.Cryptography.Encrypt(Eval("PSN_ServerID").ToString()) %>&a=<%# Eval("ApplyID")==DBNull.Value?"":Utility.Cryptography.Encrypt(Eval("ApplyID").ToString()) %>")'><%# Eval("ApplyID")==DBNull.Value?"":"打印"%></span>
                                    </ItemTemplate>
                                    <HeaderStyle HorizontalAlign="Center" Wrap="false" />
