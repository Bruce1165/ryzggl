...
{
    public partial class TrainExamRoomAllot : BasePage
    {
        /// <summary>
        /// 当前培训点信息
        /// </summary>
        ...
        protected void RefreshGrid()
        {
            ...
            if (curExamPlanID > 0)
            {
                int count = ExamPlaceAllotDAL.SelectCount(string.Format(" and ExamPlanID={0}", curExamPlanID));
                if (count == 0)
                {
                    ExamPlaceOB _ExamPlaceOB = ExamPlaceDAL.GetObjectByExamPlaceName(curTrainUnit.TrainUnitName);

                    ExamPlaceAllotOB _ExamPlaceAllotOB = new ExamPlaceAllotOB();
                    _ExamPlaceAllotOB.ExamPlaceID = _ExamPlaceOB.ExamPlaceID;
                    _ExamPlaceAllotOB.ExamPlanID = curExamPlanID;
                    _ExamPlaceAllotOB.ExamPersonNum = 0;
                    _ExamPlaceAllotOB.ExamPlaceName = _ExamPlaceOB.ExamPlaceName;
                    _ExamPlaceAllotOB.RoomNum = 0;
                    _ExamPlaceAllotOB.Status = EnumManager.ExamPlaceAllotStatus.AllotPlaced;
                    _ExamPlaceAllotOB.CreatePersonID = 0;
                    _ExamPlaceAllotOB.CreateTime = DateTime.Now;
                    ExamPlaceAllotDAL.Insert(_ExamPlaceAllotOB);
                }
            }
            ...
            q1.Add(string.Format("ExamPlanID={0}", curExamPlanID));   
        ...
        protected void RadGridExamPlaceAllot_UpdateCommand(object source, GridCommandEventArgs e)
        {
            ...
            switch (e.Item.OwnerTableView.Name)
            {
                case "ExamPlaceAllot"://考点  
                    ...
                    try
                    {
                        ...
                        for (int i = 0; i < ob.RoomNum.Value; i++)
                        {
                            ...
                            _ExamRoomAllotOB.CreatePersonID = Convert.ToInt64(curTrainUnit.UnitNo);
                        ...
                        if (e.CommandArgument.ToString() == "追加考场")//更新追加后考场数量及人数
                        {
                            CommonDAL.ExecSQL(tran, string.Format(@"
                                ...
                                where [EXAMPLACEALLOT].[EXAMPLACEALLOTID]={0}", ob.ExamPlaceAllotID, Convert.ToInt64(curTrainUnit.UnitNo), _CreateTime));
                        }
                ...
                case "ExamRoomAllot"://考场
                    ...
                    ob2.ModifyPersonID = Convert.ToInt64(curTrainUnit.UnitNo);
        ...
        protected void ButtonCreateExamCard_Click(object sender, EventArgs e)
        {
            ...
            //考场最多允许考试人数
            int allotRoomPersonNumber = ExamRoomAllotDAL.GetSumOfPersonNumber(curExamPlanID);
            ...
            queryOB2.Add(string.Format("ExamPlanID={0}", curExamPlanID));
            ...
            string ExamCount = ExamPlanDAL.GetIndexOfExamInYear(curExamPlanID);//年度第几次考试，3位
            ...
            queryOB3.Add(string.Format("ExamPlanID={0}", curExamPlanID));
            ...
            try
            {
                for (int i = 0; i < dtRoom.Rows.Count; i++)
                {
                    ...
                    for (int j = startNo; j < (startNo + examerCount); j++)
                    {
                         ...
                         _ExamResultOB.ExamPlanID = curExamPlanID;//考试计划
                        ...
                        _ExamResultOB.CreatePersonID = Convert.ToInt64(curTrainUnit.UnitNo);
                    ...
                    _ExamRoomAllotOB.ModifyPersonID = Convert.ToInt64(curTrainUnit.UnitNo);
                ...
                for (int i = 0; i < RadGridExamPlaceAllot.MasterTableView.Items.Count; i++)
                {
                    ...
                    _ExamPlaceAllotOB.ModifyPersonID = Convert.ToInt64(curTrainUnit.UnitNo);
            ...
            UIHelp.WriteOperateLog(PersonName, UserID, "分配准考证号", string.Format("考试计划：{0}，分配人数：{1}人。",
                RadTextBoxExamPlan.Text, _sumSignUp.ToString()));   
        ...
        protected void ButtonDeleteeExamCard_Click(object sender, EventArgs e)
        {
            ...
            {
                //删除准考证
                ExamResultDAL.DeleteByExamPlanID(trans, curExamPlanID);

                //设定考室状态为“已分配考场”
                ExamRoomAllotDAL.UpdateExamRoomStatus(trans, curExamPlanID, EnumManager.ExamRoomAllotStatus.AllotRoomed, Convert.ToInt64(curTrainUnit.UnitNo), DateTime.Now);

                //设定考室状态为“已分配考场”
                ExamPlaceAllotDAL.UpdateExamPlaceAllotStatus(trans, curExamPlanID, EnumManager.ExamPlaceAllotStatus.AllotPlaced, Convert.ToInt64(curTrainUnit.UnitNo), DateTime.Now);
            ...
            UIHelp.WriteOperateLog(PersonName, UserID, "取消准考证放号", string.Format("考试计划：{0}。", RadTextBoxExamPlan.Text));
        ...
        //    ExamPlanOB o = ExamPlanDAL.GetObject(curExamPlanID);
        ...
        protected void RefreshAllotHelpInfo()
        {
            if (curExamPlanID >0)
            {
                ...
                //报名缴费确认人数
                int _sumSignUp = ExamSignUpDAL.SelectCount_New(string.Format(" and ExamPlanID={0} and Status='{1}'", curExamPlanID, EnumManager.SignUpStatus.PayConfirmed));

                //待分配人数
                int waitAllot = (_sumSignUp - allotRoomPersonNumber) > 0 ? (_sumSignUp - allotRoomPersonNumber) : 0;

                LabelAllotHelp.Text = string.Format("（待考试人数{0}，已分配人数 {1}，待分配人数{2}）", _sumSignUp, allotRoomPersonNumber, waitAllot);
            }
            else
            {
                LabelAllotHelp.Text = "";
