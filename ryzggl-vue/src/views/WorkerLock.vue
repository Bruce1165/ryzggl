<template>
  <div class="worker-lock-container">
    <el-card>
      <!-- Header Actions -->
      <template #header>
        <div class="card-header">
          <span>人员锁定管理</span>
          <el-button type="primary" @click="handleCreateLock">锁定人员</el-button>
        </div>
      </template>
      ...
      <el-form :model="searchForm" :inline="true" class="search-form">
        <el-form-item label="人员ID">
          <el-input v-model="searchForm.workerId" placeholder="请输入人员ID" clearable type="number" />
        </el-form-item>
      ...
      <el-table :data="tableData" border style="width: 100%" v-loading="loading">
        ...
        <el-table-column prop="workerId" label="人员ID" width="100" />
    ...
    >
      <el-form :model="formData" :rules="formRules" ref="formRef" label-width="120px">
        <el-form-item label="人员ID" prop="workerId">
          <el-input v-model="formData.workerId" placeholder="请输入人员ID" type="number" disabled />
        </el-form-item>
    ...
    <el-dialog v-model="viewVisible" title="锁定详情" width="500px">
      <el-descriptions :column="1" border v-if="currentRow">
        ...
        <el-descriptions-item label="人员ID">{{ currentRow.workerId }}</el-descriptions-item>
        ...
        <el-descriptions-item label="锁定结束时间">{{ formatDateTime(currentRow.lockEndTime) || '-' }}</el-descriptions-item>
    ...
    <!-- Unlock Dialog -->
    <el-dialog v-model="unlockDialogVisible" title="解锁人员" width="400px">
...
const loadLocks = async () => {
  ...
  try {
    ...
    if (searchForm.workerId) {
      params.keyword = searchForm.workerId.toString()
    }
    if (searchForm.lockPerson) {
      params.keyword = searchForm.lockPerson
    }
...
const handleUnlockSubmit = async () => {
  ...
  await unlockRef.value.validate(async (valid) => {
    ...
    try {
      await unlockWorker(currentRow.value.lockId, unlockForm.unlockPerson)
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      await lockWorker({
        ...
        lockPerson: formData.lockPerson,
        lockEndTime: formData.lockEndTime,
